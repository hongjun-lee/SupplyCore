...
| 2 | NC 厂商真端点联调（Sprint 15a 仅 Mock + Bearer stub）| 中 | 1-2 PD（取决于厂商配合）|
| 3 | NcInterfaceHttpClient OAuth2 升级（一期 Bearer stub）| 中 | 1 PD（融入候选 D）|
...
| # | 决策点 | V0.1 倾向 |
|---|---|---|
| 1 | Sprint 16a 主线方向（A/B/C/D/E）| A + E 月结反结 双轨（完整闭环详设 08） |
| 2 | 4 累计技术债哪些必修 | 全修（~3 PD）|
| 3 | 工时预算 | 12-15 PD（同 Sprint 13a/14a/15a 经验）|
| 4 | 子代理并行策略 | 主+2 子代理 sweet spot 3.8x |
| 5 | Codex 15a 评审时机 | Sprint 15a 收尾后 cici 触发 |
