# Sprint 19c 任务卡 V0.2（锁版）

**项目：** 阜矿物资供应管理系统 / SupplyCore
**版本：** V0.2（锁版 · cici 2026-05-15 选双轨 UI-2 + A2' + X1 旁路）
**日期：** 2026-05-15
**文档性质：** 实施层 · Sprint 19c 锁版任务卡
**配套：** [`Sprint-19b-Demo-脚本-V0.1.md`](./Sprint-19b-Demo-脚本-V0.1.md) §四 候选范围、Sprint-19c-V0.1（已锁定）

---

## 一、Sprint 19c 主线方向（锁版 · 双轨 UI-2 + A2' + X1）

### 主轨 UI-2：6 MVP 页面详情页 + 后端 endpoint 配套
...
### 副轨 A2'：NC 真端点联调 phase 2（19a/19b 二次顺延 / 19c 第三次保留）

| Task | 范围 | 工时 | 依赖 NC 端 |
|---|---|---|---|
| 19c-A2-1~4 | 同 19b A2'（NC 端配合度 / OAuth2 真接通 / 23 接口实测 / BIZ-005A 灰度）| 4 PD | **强** |

### 旁路 X1：OAuth2 Token Redis 持久化缓存（17a/19a/19b 三次顺延 / 19c 收编）
...
| # | 决策点 | V0.2 锁版结论 |
|---|---|---|
| 1 | Sprint 19c 主线方向 | **双轨 主轨 UI-2 + 副轨 A2' + 旁路 X1**（cici 选 推荐方案）|
| 2 | 5 累计技术债 | **全修** ~10-12 PD（UI-2 主线吸收 #1-#3 / X1 旁路 #4 / A2' 副轨 #5）|
...
| 6 | A2' 第三次顺延决策 | **保留 A2' 副轨 + 19d 前 cici 评估硬截止 vs 撤 A2'**（避免长期"伪保留"）|
...
| **V0.2** | **2026-05-15** | **cici 评审锁版双轨 UI-2 + A2' + X1 旁路**：主轨 UI-2 详情页 + 后端 endpoint 配套 6.5-7 PD / 副轨 A2' NC 反馈触发 4 PD / 旁路 X1 OAuth2 Redis 0.5 PD = 11-13 PD；主+子代理 b+c sweet spot 3x；A2' 19d 前 cici 评估硬截止 |
