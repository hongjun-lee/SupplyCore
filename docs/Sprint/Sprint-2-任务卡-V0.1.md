# Sprint 2 任务卡 — P-04/P-05 + 合同三件套 + P-02→P-05 linkage（V0.1）
...
- 上游工时模型 → [`开发进度规划-V0.4.md`](../详细设计/开发进度规划-V0.4.md) §3.2 04 / 05 子模块
...
| D2-2 | 业务规则：`adj_type=取消行` 须 P-03 `fulfillment_state=待采购`；Approve 后服务层更新对应 P-03 行 | 04 §4.6.2 业务规则 2-3 | 单测覆盖 happy + reject 两路径 |
...
| D7-2 | 多方会签业务规则：每签收人审批一次推进节点（接 Catio Workflow，先 mock） | 05 §4.1.3 + 10A | 单测 + log 验证 |
...
| D9-3 | NC 接口预留位（详设 05 §4.2 + 详设 08 §5.2 BIZ-001/004）；本期仅声明 endpoint stub | 详设 08 + 08A 给NC清单 V0.1 | 接口 stub DI 可解析 |
