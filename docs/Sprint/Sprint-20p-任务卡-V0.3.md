# Sprint 20p 任务卡 V0.3（2026-05-18 cici D-3 拍板 ProcurementDocument 术语对齐 / 招标 7 类文件协调 + 试点验证 sprint / 2026-05-18 启动 Day 1）

**Sprint**：20p（紧续 20o 真正收口 → 第 4 周期 5 Sprint 第 1 阶段开局）
**主题**：**招标文件 7 类协调 + ProcurementDocument 试点验证**（ProcurementDocument = 招标文件 / 不是采购订单 / cici D-3 术语对齐拍板）+ Tender↔Contract↔Approval 关联完整性 grep + UI polish + 试点扩大白音华煤矿 + 试点 demo
**节奏**：roadmap V0.1 §2.1 撤销「从零开发 PurchaseOrder」/ V0.2 修正为「协调 + 试点验证 + UI 完善 sprint」/ **D-3 cici 拍板**：ProcurementDocument 术语为「招标文件」（7 类 / 资格预审/投标书/评标报告/中标通知书等 / 非采购订单）/ 工作量 ~1.2 PD（vs roadmap §2.1 估 2.0 PD / 省 0.8 PD）
...
| **T-E2** 招标文件业务方 demo walk-through page | 0.2 | pending | 类 Sprint 20n T-E4 pilot-demo 模式 / 5 步引导（招标文件 7 类）|
...
| 维度 | Sprint 20n（库存协调）| **Sprint 20p（招标文件协调）** |
|---|---|---|
...
| 主轴战略 | 库存试点 Wave 1 开局 | 招标文件试点 Wave 1 开局 |
...
| **Q2** | second e 副轨范围 | A. 启动 T-E1+T-E2+T-E3（3 task / 0.7 PD）/ B. 仅 T-E1 polish（0.3 PD / 简化）/ C. 暂停（类 20o）| **A 默认**（招标文件 UI polish 真正影响试点验证 / second e 第 12 次连续 / 模块切到招标文件）|
...
- **second e 第 12 次连续 / 模块 8/8 已完整 / 副轨可能找不到高价值 task**：cici Q2 决策 second 副轨范围
  * **缓解**：Q2 A 选项启动 T-E1+T-E2+T-E3 / 招标文件模块 frontend polish 真正影响试点验证 / 模块切换成本评估
...
> Sprint 20p 第 4 周期第 1 阶段开局：**ProcurementDocument（招标文件 7 类）试点验证** + 4 模块关联完整性 grep + 白音华煤矿试点扩大协调 + 招标文件业务方 demo 准备 / 43 Sprint 0 顺延维持。第 3 批 4 模块（数据治理+库存+招标文件+合同）production-ready 中间点验证持续。**D-3 cici 拍板术语对齐：ProcurementDocument = 招标文件 / 非采购订单**。roadmap V0.1 §2.1 撤销「从零开发」转为「协调 + 试点验证 + UI 完善 sprint」/ 工作量 2.0 → 1.2 PD / T-A5 同步修正 roadmap V0.2。
...
| 版本 | 日期 | 变更 |
|---|---|---|
| V0.3 | 2026-05-18 | **cici D-3 拍板 ProcurementDocument 术语对齐**：① 头部 V0.2 → V0.3 / ② 主题加「招标文件 7 类协调 + ProcurementDocument 试点验证」/ ③ 节奏行加 D-3 术语注（ProcurementDocument = 招标文件 / 7 类 / 非采购订单）/ ④ task 描述「采购模块/采购流程/采购业务方」全改为「招标文件模块/招标文件流程/招标文件业务方」/ ⑤ 节奏表「采购协调」→「招标文件协调」/ ⑥ 对外汇报加术语对齐说明 / **目的**：避免「采购订单」歧义 / 实际为招标 7 类文件（资格预审/投标书/评标报告/中标通知书等）|
