# Sprint 5a 任务卡 — 招投标闭环 + 库存出库前置（V0.2）

**项目：** 阜矿物资供应管理系统 / SupplyCore
**版本：** V0.2（评审后锁版 / 实施基线）
**日期：** 2026-05-13
**文档性质：** 开发实施层 · Sprint 任务卡（实施基线）
**适用范围：** 后端工程 `SupplyCores` 仓库 Sprint 5a（预估 10 工作日 / 约 2 周）
**并行轨道：** 与 Sprint 5b 外委检修专项 平行进行（详 [`Sprint-5b-外委检修专项-任务卡-V0.2.md`](./Sprint-5b-外委检修专项-任务卡-V0.2.md)）
...
Sprint 5a 聚焦 **T 域闭环** + **S 域出库前置**，与 Sprint 5b（E 域 + 外委检修）几乎正交，仅共享 DbContext / ModelSnapshot 治理协议。
...
- Sprint5a_TenderFullChain_E2E：T-01 → T-03 → T-04 → T-06 LogAsync → T-08 → T-09 → T-05 → C-02
- T-06 招采平台对接 mock 端到端 batch 接入测试（导入/导出 双向）

**C. 库存出库前置（~3.5 PD，Sprint 6a 主线前移）**
- S-06 PurchaseReturn 采购退货（详设 06 §4.6）：5 状态 + 与 S-05 入库联动
- S-09 MaterialIssue 领料出库（详设 06 §4.7）：6 状态 + 库存扣减

**D. 验收 + Sprint 6a backlog（~1.5 PD）**
- 全量回归 ≥ 410 通过
- Sprint-5a-Demo-脚本-V0.1
- Sprint-6a 任务卡草案（S-10 退料入库 / S-12 跨组织调拨 / 招采平台真接 / C-07 付款计划起步）
...
| 4 | 与 b 集成测试边界 | **a 不依赖 b** | 两轨道完全正交；b 升详设 07 不影响 a |
| 5 | E2E 写法 | **单文件 Sprint5a_FullChain** | 单测式 E2E 与 Sprint 3/4 模式一致 |
| 6 | 总工时上限 | **严卡 10 PD** | Sprint 4 已经历质量债教训；不再用超额换范围 |

### 1.3 不在范围

- Sprint 5b 外委检修专项（独立轨道）
- S-10 退料入库 / S-12 跨组织调拨（留 Sprint 6a）
- C-07~C-10 付款流程（移 Sprint 6a）
- 招采平台真实对接（依赖外部平台凭据，留 Sprint 6a+ 机会窗口）
- 09 报表预警 AI 能力（跨 Sprint 大模块）
- E 域设备模块（由 Sprint 5b 起步）
...
### 1.5 完成标准（Sprint 5a 验收）

- [ ] §二 D1-D10 全部任务 ✅
- [ ] 全量测试 ≥ 410 通过（基线 370 + a 新增 ~40）
- [ ] 新增 EF migrations 4 条：Add_Tender_T08_T09 / Add_StockOutbound_S06 / Add_MaterialIssue_S09 / Add_S04_S05_S06_Linkage（如有）
- [ ] Sprint5a_TenderFullChain_E2E + Sprint5a_OutboundChain_E2E 通过
- [ ] Sprint-5a-Demo 入库
- [ ] **b 集成回归**：在 a 收尾后，与 b 主分支 merge 后全量再回归一次
...
| # | 任务 | 详设引用 | 验收 |
|---|------|---------|------|
| D5-1 | Sprint5a_TenderFullChain_E2E：T-01 → T-03 + T-04 → T-06 LogAsync（导入 batch）→ T-08 提交 + 撤回 + 评定 → T-09 评分 → T-05 中标 → C-02 已签 | 04 全链 | E2E 1 个通过 |
...
| # | 任务 | 验收 |
|---|------|------|
| D9-1 | Sprint5a_OutboundChain_E2E：S-05 入库 → S-06 部分退货 + S-09 部分领料 → C-02 executed_amount 反向核对 | E2E 通过 |
| D9-2 | sub_group_id 守护单测自动覆盖 a 新增 4 实体（T-08/T-09/S-06/S-09）| 守护单测全过 |
| D9-3 | 全量回归 ≥ 410 通过 | 0 失败 |
| D9-4 | **与 b 主分支 merge 集成回归**：本地 merge b 最新 push 后再跑一次全量 | 集成 0 失败 |

**预估工时：** 1 PD

### Day 10 — Demo + Sprint 6a 草案（~0.5 PD）

| # | 任务 | 验收 |
|---|------|------|
| D10-1 | 全量测试 ≥ 410 通过 | 0 失败 |
| D10-2 | Sprint 1-5 Demo 用例 1-22 回归 + Sprint 5a 新增用例 23-26（T-08/T-09/S-06/S-09）| 全 200 OK |
| D10-3 | 写 `Sprint-5a-Demo-脚本-V0.1.md` | 入库 |
| D10-4 | 起 Sprint 6a 任务卡草案：S-10 退料入库 / S-12 跨组织调拨 / 招采平台真接 / C-07 付款计划起步 | `Sprint-6a-任务卡-V0.1.md` 草案 |
| D10-5 | 整理 Sprint 5a commit log | git log 整洁 |

**预估工时：** 0.5 PD

**Sprint 5a V0.1 总工时（草案）：** 2 + 2 + 1 + 1.5 + 1.5 + 1 + 0.5 + 余 0.5 buffer = **10 PD ✓**
...
| 风险 | 概率 | 影响 | 缓解 |
|------|------|------|------|
| 与 b 共享 ModelSnapshot.cs 冲突 | 高 | 每次 push 前需 rebase | 协议：晚上 push 前 `git pull --rebase`，冲突时重 add migration |
| Wave 编号撞车 | 低 | EF 表名重复 | 预分配 a=27-30 / b=31-38；越界前同步 |
| T-08 业务规则细节（撤回时点 / 评定标准）业务方未确认 | 中 | D1-D2 字段返工 | D1 第一动作业务方对齐；不确认即按详设最小可用版落地 |
| S-06 / S-09 库存余量校验需 S-13 库存余额表 | 高 | S-13 未落地，余量取不到 | Sprint 5a 简化为：S-06 用 S-05 入库剩余可退量校验（不依赖 S-13）；S-09 用 mock 余量校验（Sprint 6a 接 S-13）|
| b 主分支推进过快导致 a D9-4 集成冲突 | 中 | 集成回归延期 | 每周五双方同步基线 commit；不让单方主分支超 3 天领先 |
...
| 版本 | 日期 | 变更 |
|------|------|------|
| V0.1 | 2026-05-13 | 首版草案，基于 Sprint 5 V0.2 D0-D2 已落基线起。Sprint 级 a/b 并行方案确认后单独成卡。范围 4 类候选：A 招投标 T-08/T-09 / B 主链 E2E / C 出库 S-06+S-09 前置 / D 验收，约 10 PD。6 决策点待用户评审 + b 共享治理协议见 §四 风险表。 |
| V0.2 | 2026-05-13 | 评审 6 决策点一次性按"我倾向"锁版：(1) 全 T-08+T-09；(2) T-06 batch 端到端 mock；(3) S-06 + S-09 都做；(4) a 不依赖 b；(5) 单文件 Sprint5a_FullChain；(6) 严卡 10 PD。§二 D1-D10 工时不变（2+2+1+1.5+1.5+1+0.5 = 10 PD ✓）；a 即刻进入实施，按 D1 起步 T-08。**入实施时发现 V0.2 引用错位**（D1 准备阶段）：详设 04 V1.1 §4.12 实际是 T-05，无 T-08 / T-09 实体表定义。同步升详设 04 V1.1 → V1.2（commit B 选项），§4.13a/§4.13b 补齐 T-08/T-09 全字段表 + 状态机 + 业务规则；a V0.2 引用同步更新为 V1.2 §4.13a/§4.13b。本次任务卡章节号不重命名（V0.2 主体内容仍生效），仅更新详设引用。|
