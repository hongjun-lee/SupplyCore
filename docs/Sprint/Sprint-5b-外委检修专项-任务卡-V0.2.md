# Sprint 5b 任务卡 — 外委检修专项 + 4 审批模板（V0.2）

**项目：** 阜矿物资供应管理系统 / SupplyCore
**版本：** V0.2（评审后锁版 / 实施基线）
**日期：** 2026-05-13
**文档性质：** 开发实施层 · Sprint 任务卡（实施基线）
**适用范围：** 后端工程 `SupplyCores` 仓库 Sprint 5b（预估 10 工作日 / 约 2 周）
**并行轨道：** 与 Sprint 5a 招投标闭环 平行进行（详 [`Sprint-5a-招投标闭环-任务卡-V0.2.md`](./Sprint-5a-招投标闭环-任务卡-V0.2.md)）
...
Sprint 5b 聚焦 **E 域设备起步** + **外委检修业务闭环（40% 上限 + 高敏感拦截器 + 4 审批模板）**，与 Sprint 5a（T 域 + S 域出库）几乎正交，仅共享 DbContext / ModelSnapshot 治理协议。

**前置发现**：详设 07 已是 V1.0a（含外委检修字段 + 40% 上限配置项），**Sprint 5b 不需要升 07**；可能需 05 V1.2 → V1.3 小升把 `equipment_id` 字段落入 C-02 §4.2.1。
...
**F. 集成 + Demo + Sprint 6b 草案（~2.5 PD）**
- Sprint5b_RepairChain_E2E：E-01 设备 → E-05 申请 → 审批通过 → C-02 合同（含 service_subtype=外委检修 + 40% 校验）→ 超阈值场景触发 SENS-CON-004
- 全量回归 ≥ 410 通过
- Sprint-5b-Demo-脚本-V0.1
- Sprint-6b 任务卡草案（设备完整 E-03~E-12 + 暂估双预警 + 后评价联动）
...
| 2 | E 域最小可用 | **E-01 + E-02 都落** | E-01 是 40% 上限计算基数源；E-02 字典轻量同期落避免后续 Sprint 6b 字典补建 |
...
| 6 | b/a 集成边界 | **b 不依赖 a** | 两轨道完全正交；b E2E 不引用 a T-08 评标流程 |
| 7 | 总工时上限 | **严卡 10 PD** | 与 a 一致；不再用超额换范围 |

### 1.3 不在范围

- Sprint 5a 招投标闭环（独立轨道）
- E-03~E-12 设备完整模块（留 Sprint 6b）
- 设备租赁闭环（详设 07 §4.2，留 Sprint 6b+）
- C-07~C-10 付款流程（a/b 都不做，留 Sprint 6+）
...
| # | 任务 | 验收 |
|---|------|------|
| B6-1 | Sprint5b_RepairChain_E2E：E-01 设备 → E-05 申请 → 审批通过 → C-02 合同（service_subtype=外委检修 + EquipmentId）+ 40% 上限校验 happy + 超阈值场景（SENS-CON-004 attribute 触发） | E2E 通过 |
| B6-2 | sub_group_id 守护单测自动覆盖 b 新增 3 实体（E-01/E-02/E-05）| 守护单测全过 |
| B6-3 | 全量回归 ≥ 410 通过 | 0 失败 |
| B6-4 | **与 a 主分支 merge 集成回归**：本地 merge a 最新 push 后再跑一次全量 | 集成 0 失败 |

**预估工时：** 1 PD

### Day 10 — Demo + Sprint 6b 草案（~0.5 PD）

| # | 任务 | 验收 |
|---|------|------|
| B7-1 | 全量测试 ≥ 410 通过 | 0 失败 |
| B7-2 | Sprint 1-5 Demo 用例回归 + Sprint 5b 新增用例 27-30（E-01/E-05/40% 上限/SENS-CON-004）| 全 200 OK |
| B7-3 | 写 `Sprint-5b-Demo-脚本-V0.1.md` | 入库 |
| B7-4 | 起 Sprint 6b 任务卡草案：设备完整 E-03~E-12 + 设备租赁闭环 + 后评价联动 + 暂估双预警 | `Sprint-6b-任务卡-V0.1.md` 草案 |
| B7-5 | 整理 Sprint 5b commit log | git log 整洁 |

**预估工时：** 0.5 PD

**Sprint 5b V0.1 总工时（草案）：** 0.5 + 1.5 + 2.5 + 2 + 1 + 1 + 0.5 + 余 1 buffer = **10 PD ✓**
...
| 风险 | 概率 | 影响 | 缓解 |
|------|------|------|------|
| 与 a 共享 ModelSnapshot.cs 冲突 | 高 | 每次 push 前需 rebase | 协议：晚上 push 前 `git pull --rebase`，冲突时重 add migration |
| Wave 编号撞车 | 低 | EF 表名重复 | 预分配 a=27-30 / b=31-38；越界前同步 |
...
| 40% 上限校验依赖 Equipment.OriginalValue 真实数据 | 中 | seed 仅 mock 数据测试 | Sprint 5b 单测/E2E 用 mock；Sprint 6b 接入真实设备档案 |
| a 主分支推进过快导致 b B6-4 集成冲突 | 中 | 集成回归延期 | 每周五双方同步基线 commit；不让单方主分支超 3 天领先 |
| SY-02 `OUTSOURCED_REPAIR_PRICE_CAP_RATIO` 未落地 seed | 中 | 校验逻辑取不到配置 | Sprint 5b B4-3 时同步加 SY-02 seed |
...
| 版本 | 日期 | 变更 |
|------|------|------|
| V0.1 | 2026-05-13 | 首版草案，基于 Sprint 5 V0.2 D0-D2 已落基线起。Sprint 级 a/b 并行方案确认后单独成卡。范围 6 类（A 详设小升 / B E 域设备主档 / C E-05 外委检修申请 / D 40% 上限 + SENS-CON-004 / E 4 审批模板 / F 集成 Demo）总 ~10 PD。7 决策点待用户评审 + a 共享治理协议见 §四 风险表。前置发现：详设 07 V1.0a 已含外委检修字段，本 Sprint 不升 07；可能升 05 V1.2 → V1.3 小版（加 C-02 equipment_id / overlimit_*）。|
| V0.2 | 2026-05-13 | 评审 7 决策点一次性按"我倾向"锁版：(1) 05 V1.2 → V1.3 小升；(2) E-01+E-02 都落；(3) E-05 6 状态完整；(4) 仅 C-02 超阈值 SENS-CON-004；(5) 4 审批模板全 mock seed；(6) b 不依赖 a；(7) 严卡 10 PD。§二 D1-D10 工时不变（0.5+1.5+2.5+2+1+1+0.5 = 9 PD + 1 PD buffer）；b 即刻进入实施，按 B1 起步详设 05 V1.3 升版。|
