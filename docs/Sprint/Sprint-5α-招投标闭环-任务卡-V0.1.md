# Sprint 5α 任务卡 — 招投标闭环 + 库存出库前置（V0.1）
...
**并行轨道：** 与 Sprint 5β 外委检修专项 平行进行（详 [`Sprint-5β-外委检修专项-任务卡-V0.1.md`](./Sprint-5β-外委检修专项-任务卡-V0.1.md)）
...
**A. 招投标投标响应 + 评标结果（~4 PD）**
- T-08 BidResponse 投标响应（详设 04 §4.12）：4 状态（提交 / 撤回 / 评标中 / 评定）
- T-09 EvaluationResult 评标结果明细（详设 04 §4.14）：关联 T-08，无状态
...
| # | 决策点 | 选项 |
|---|--------|------|
| 1 | 招投标范围 | (a) 全 T-08+T-09 / (b) 仅 T-08（T-09 留 Sprint 6α）/ (c) T-08 + T-09 字段对齐但 T-09 不上 AppService |
| 2 | T-06 联调 | (a) 加 batch 端到端 mock（导入/导出全场景）/ (b) 仅 LogAsync 单点 unit 测试 |
| 3 | 出库切片 | (a) S-06 + S-09 都做 / (b) 仅 S-06 / (c) 仅 S-09 / (d) 都留 Sprint 6α |
| 4 | 与 β 集成测试边界 | (a) α 不依赖 β（默认）/ (b) α 加 service_subtype 字段引用检查（依赖 β 升详设 07） |
| 5 | E2E 写法 | (a) 单文件 Sprint5α_FullChain（推荐）/ (b) 多文件按子链拆分 |
| 6 | 总工时上限 | (a) 严卡 10 PD（默认）/ (b) 接受 11 PD（如 S-06 + S-09 双出库压紧） |

**我倾向** ：1-(a) / 2-(a) / 3-(a) / 4-(a) / 5-(a) / 6-(a) — 严卡 10 PD。出库 S-06+S-09 两个一起做有协同效应（共享库存事务模式）。
