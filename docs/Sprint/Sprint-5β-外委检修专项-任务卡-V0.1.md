# Sprint 5β 任务卡 — 外委检修专项 + 4 审批模板（V0.1）
...
**并行轨道：** 与 Sprint 5α 招投标闭环 平行进行（详 [`Sprint-5α-招投标闭环-任务卡-V0.1.md`](./Sprint-5α-招投标闭环-任务卡-V0.1.md)）

**衔接文档：**

- 上游 Sprint → [`Sprint-5-任务卡-V0.2.md`](./Sprint-5-任务卡-V0.2.md)（D0-D2 已落基线）
- 详设依据：07 V1.0a §五（E-01 / E-05 含外委检修字段）+ §八（40% 上限配置项）/ 05 V1.2 §8.7 / 10 V1.2 §7.1（SENS-CON-004）
...
### 1.2 V0.1 评审决策点（待用户拍板）

| # | 决策点 | 选项 |
|---|--------|------|
| 1 | 详设升版范围 | (a) 05 V1.2 → V1.3 小升（加 C-02 equipment_id + overlimit_*）/ (b) 不升详设，equipment_id 走 V1.3 时统一回填字段表 / (c) 升 05 V1.3 + 07 V1.0a → V1.1（含 E 域完整字段化） |
| 2 | E 域最小可用 | (a) E-01 + E-02 都落 / (b) 仅 E-01（E-02 留 Sprint 6β） / (c) E-01/E-02/E-08 附件三件套（参考详设 07 §五附件留存）|
| 3 | E-05 状态机 | (a) 6 状态（含进行中 / 已完工）/ (b) 4 状态（草稿/待审/已审/已驳回，进行中-已完工 留 Sprint 6β）|
| 4 | SENS-CON-004 拦截范围 | (a) 仅 C-02 超阈值（默认）/ (b) C-02 + E-05 申请同时拦（双校验）|
| 5 | 4 审批模板配置 | (a) 4 个全 mock seed / (b) 仅 WF-CON-OVERLIMIT-001 起步 / (c) WF-RPR + WF-CON-OVERLIMIT 二选 + WF-DIR/WF-SUP-REASSESS 留 Sprint 6β |
| 6 | β/α 集成边界 | (a) β 不依赖 α（默认）/ (b) β E2E 引用 α T-08 评标流程（强依赖）|
| 7 | 总工时上限 | (a) 严卡 10 PD（默认）/ (b) 接受 11 PD（如 E 域全 3 件套 + 4 模板都落）|

**我倾向**：1-(a) / 2-(a) / 3-(a) / 4-(a) / 5-(a) / 6-(a) / 7-(a) — 严卡 10 PD。E-01+E-02 都落、E-05 6 状态完整、4 审批模板全 mock 配。
...
- ✅ 详设 05 V1.2（含 service_subtype 字段 + §8.7 40% 上限规则）
...
- [ ] 详设 05 V1.2 → V1.3 升版完成（如 1-(a) / 1-(c) 通过）

---

## 二、按日任务拆解（V0.1 草案，按"我倾向"起草）
...
| B4-3 | Contract.Approve() 内加 40% 上限校验：当 ServiceSubtype="外委检修" + EquipmentId 非空 时，ContractAmount ≤ Equipment.OriginalValue × SY-02 `OUTSOURCED_REPAIR_PRICE_CAP_RATIO`（默认 0.40）；超阈值需 OverlimitReason 必填 + 触发 WF-CON-OVERLIMIT-001 | 详设 05 V1.2 §8.7 + 07 §五业务规则 5 | 单测 ≥ 4（含负向 + 边界） |
...
| B5-3 | WF-CON-OVERLIMIT-001 合同超阈值审批模板配置 seed | 详设 05 V1.2 §8.7.2 + 10 §4.6 | seed 加载 |
