# Sprint 6a 任务卡 — 供应链下游补强 + 招采平台真接（V0.1 草案）
...
**并行轨道：** 与 Sprint 6b 设备运维强化 平行进行（草案另立）
...
- Sprint 5a 落地基线 → commit 待 D10 收尾（含 Sprint 5a / 5b 合并集成回归）
...
| # | 决策点 | 候选方案 | 倾向 |
|---|--------|----------|------|
| 1 | S-10 退料入库范围 | A. 完整 5 状态 / B. 简化为 3 状态（草稿/已审/已退）| A — 与 S-09 出库 6 状态对齐 |
| 2 | 招采平台真接时机 | A. 本期做 / B. 顺延 Sprint 7a | A — OAuth 凭据 Sprint 4 D9 已拿到，不接成本累积 |
| 3 | C-07 自动生成 vs 手动 | A. C-04 满足后自动生成 / B. 全部手动 | A — 详设 05 §4.7 明确为自动联动 |
| 4 | C-08 NC 资金接口范围 | A. BIZ-PAY stub + 实体落 / B. 仅实体落，stub 延后 | A — 沿用 BIZ-RED / BIZ-001 stub 模式成本低 |
| 5 | 与 b 集成测试边界 | A. a 不依赖 b / B. a 接 b 的 E-05 RepairApplication 联动 C-02 | 待 Sprint 5b 完成后确认（b 详设 07 V1.0b 升版进度决定） |
...
### 1.4 基线（待 Sprint 5a/5b 收尾确认）

- ☐ Sprint 5a D10 commit 已 push + Demo-5a 入库
- ☐ Sprint 5b 全部任务收尾 + Demo-5b 入库
- ☐ 详设 07 V1.0b 升版（拆 E-05 RepairApplication + EquipmentInspectionRecord 两实体）— Sprint 5b 收尾决策点
- ☐ EF migrations ≤ 32 条全部 apply（5a 加 4 / 5b 加 5）
- ☐ sub_group_id 守护单测覆盖 Sprint 5a/5b 新增实体

### 1.5 完成标准（Sprint 6a 验收）

- [ ] §二 D1-D10 全部任务 ✅
- [ ] 全量测试 ≥ 580 通过（基线 ~540 + 6a 新增 ~40）
