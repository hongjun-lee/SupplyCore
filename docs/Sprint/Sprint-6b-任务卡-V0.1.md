# Sprint 6b 任务卡 — 设备运维强化 + 详设 07 V1.0b 升版 + 租赁闭环起步（V0.1 草案）
...
**并行轨道：** 与 Sprint 6a 供应链下游补强 平行进行（草案 [`Sprint-6a-任务卡-V0.1.md`](./Sprint-6a-任务卡-V0.1.md)）
...
- Sprint 5b 落地基线 → commit 待 B7 收尾（含 Sprint 5b 全 7 任务收尾）

---

## 一、目标与范围

### 1.1 V0.1 候选范围（约 10 PD，待评审收口到 ~8）
...
| # | 决策点 | 候选方案 | 倾向 |
|---|--------|----------|------|
| 1 | E-05 代码层拆分时机 | A. 本期拆 / B. 顺延到详设 V1.0b 稳定后 Sprint 7b | A — Sprint 5b B3 升版决策已识别，越拖兼容成本越高 |
| 2 | Equipment 7 状态扩展范围 | A. 完整 7 状态 + 转换矩阵 / B. 增量 5 状态（不含租赁/外委）| A — 与设备租赁闭环 E-09 同期落地 |
| 3 | E-03 厂家字典数据来源 | A. 独立实体 / B. M-09 Supplier 子集（manufacturer_flag）| A — 独立字典便于设备主档关联与统计 |
| 4 | E-11 LeaseBilling NC 接口 | A. 本期 stub / B. 顺延 Sprint 7b | B — 业务规则未稳定（按日/按月口径），先落实体 |
| 5 | 与 a 集成测试边界 | A. b 独立 / B. b 接 a 的 S-09 领料联动 E-06 备件 | A — Sprint 6b 不引入跨域联动，留 Sprint 7b |
| 6 | E-02 命名最终确认 | A. 保留 EquipmentCategory / B. 改名 EquipmentClassification | A — 已大量代码引用，重命名收益不抵成本 |
...
### 1.4 基线（待 Sprint 5b 收尾确认）

- ☐ Sprint 5b B7 commit 已 push + Demo-5b 入库
- ☐ Sprint 5a 全部任务收尾 + Demo-5a 入库
- ☐ 详设 07 V1.0a 已锁版（V1.0b 升版作为本 Sprint A 任务交付）
- ☐ EF migrations ≤ 41 条全部 apply（5b 加 4 / 5a 加 4）
- ☐ sub_group_id 守护单测覆盖 Sprint 5b 新增 3 实体（Equipment / EquipmentCategory / RepairApplication）
