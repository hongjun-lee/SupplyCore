...
**V0.1 内修订记录**（2026-05-17 当日 / Spencer 二~六轮反馈）：
...
- **七轮反馈 → material_code 第 1 阶段不再 = legacy_code**（§六.3 修订 / §八-A.8.3 简化）：业务方既然预先重新编码大类-小类，原 `legacy_code` 的分类前缀就与新分类对不上 → 第 1 阶段就应该按新规则生成 `material_code = M-XX-XX-NNNN`（不能再"业务方零适应"）。SupplyCore 按业务方映射表自动生成 material_code / 工程师 SQL 只出 legacy_code（更省事）
...
| 3 | 迁移范围 | **试点 3 单位先抽**（`001.007.001` 本部 + `001.007.002` 恒大煤矿 + `001.007.018` 物资公司）|
...
| 5 | 试点 3 单位在原系统的真实数据量（评估抽取性能）| 可工程师在 SQL 内 COUNT |
...
```sql
WHERE org_code IN ('001.007.001', '001.007.002', '001.007.018')
```
...
| 类型 | 预估量级 | 备注 |
|---|---|---|
| 02 物料编码 | 1500-3000 | 比集团全量 5000-10000 缩小 60-70% |
| 03 供应商 | 300-500 | 试点 3 单位常用供应商 |
| 04 仓库 / 库区 / 货位 | 50 / 200-400 / 1000-2000 | 货位部分需现场盘点 |
| 06 期初库存 | 5000-15000 行 | 万行级，人工填不现实 |
...
| 目标字段 | 类型 | 原系统字段（占位）| 备注 |
|---|---|---|---|
| material_code | string(32) | = legacy_code | 第 1 阶段沿用原编码 |
| **legacy_code** | string(32) | `MATERIAL.MAT_CODE` | **本方案新增 / 双轨追溯 / 唯一索引（可空）** |
...
| category_code | string(16) | `MATERIAL.CATE_CODE` |  |
...
```csharp
b.Property(x => x.LegacyCode).HasMaxLength(32);
b.HasIndex(x => x.LegacyCode)
 .IsUnique()
...
| **第 1 阶段（试点 1-2 周）** | `material_code = XX-XX-XXXXXX`（**规范 V1.8 格式 / 大类拼音 + 分类 2 位数字 + 流水 6 位**，例 `ZH-01-000001`）/ SupplyCore 按规范自动生成 / `legacy_code` 保留原系统编码 | 业务方适应新编码 / 但**老查询通过 legacy_code 100% 兼容** |
...
> **每个 SELECT 字段都用 `AS` 起新表英文字段为别名**（如 `m.原系统编码字段 AS material_code`），
...
SELECT DISTINCT
  ...
  m.MAT_CODE          AS material_code,   -- 第 1 阶段沿用
  m.MAT_NAME          AS material_name,
  m.SPEC              AS specification,
  m.UNIT_CODE         AS unit,
  m.CATE_CODE         AS category_code,
...
WHERE i.ORG_CODE IN ('001.007.001', '001.007.002', '001.007.018');
```

> JOIN INVENTORY 是为了过滤"试点单位实际有库存或用过的物料"，避免抽出大量历史无用物料 — 也可改用 `WHERE EXISTS` 写法，按 schema 性能挑。
...
| 校验项 | 检查口径 | 严重度 |
|---|---|---|
| 必填字段完整率 | material_code / name / unit / category_code | 高 |
| 编码唯一性 | material_code + legacy_code 唯一 | 高 |
...
```mermaid
flowchart TD
  S0a[阶段 0a：项目组出分类基线 V0.1<br/>8 大类 + 35 个二类<br/>main 主代理 + cici]
  S0b[阶段 0b：业务方核对基线 → V0.2<br/>物资公司 + 各厂矿物资部<br/>核对 / 拆并 / 提调整]
  S0c[阶段 0c：物资公司 + cici 签字锁定<br/>进 MaterialCategory 表]
  S1[阶段 1：业务方维护原→新分类映射 Excel<br/>物资公司]
...
> **详细规划见独立文档** [`物料分类基线-V0.1.md`](./物料分类基线-V0.1.md)（含 8 大类 + 35 个二类完整清单 + 业务方核对路径 + 工程师提报机制）

#### 8-A.8.3 业务方分类编码调整 + 物料 material_code / category_code 映射（V0.1 七轮锁定）

**业务方阶段 0 输出 2 份 Excel**：

| Excel | 内容 | 必要性 |
|---|---|---|
| ① 目标分类树 Excel | 新的大类-小类-子类编码 / 名称 / 上下级 / 高敏感属性 | ✅ 必出 |
| ② 原→新 分类编码映射 Excel | `original_category_code → new_category_code`（每行一条对照）| ✅ 必出（用于 02 物料 SQL JOIN）|
...
原系统工程师 SQL 输出：
  ...
  ↓
SupplyCore 接收 → JOIN 业务方映射 Excel ②
  ↓ original_category_code → new_category_code
  ↓
按 new_category_code（规范 V1.8 / 如 ZH-01）+ 流水号自动生成 material_code = XX-XX-XXXXXX
  ↓
落库到 Material 表：
  material_code = ZH-01-000001（规范 V1.8 格式 / 业务方主用）
  legacy_code   = 原系统编码（追溯用 / 老查询走这个）
  category_code = new_category_code（规范 V1.8 的 4 位分类码，如 ZH-01）
```
...
| 阶段 | 工作项 | 责任 | 工作量 |
|---|---|---|---|
| 0a | **项目组起草分类基线 V0.1**（八大类 + 35 个二类）✅ 已起草 | main 主代理 | 已完成（0 天）|
...
| 1 | 业务方编辑原→新 分类编码映射 Excel | 物资公司物资管理 + 各厂矿 | 1 天 |
...
项目组起草的分类基线 V0.1（35 个二类）再专业，也不可能 100% 覆盖原系统所有物料。原系统工程师在 SQL 归类映射时，最容易在第一时间发现"找不到合适分类"的物料 — **他们的提报反馈循环越快，分类基线越完善**。
...
| 网络 / 权限 / 性能瓶颈 | 试点 3 单位数据量小 / 可控 / 出问题立即回滚 batch |
| 双轨过渡期混乱 | 第 1 阶段 `material_code = legacy_code` / 业务方无感 |
...
> 关键路径分析：
> - **阶段 0 物料分类前置确认是 02 物料 SQL 抽数的强阻塞项**（分类不锁，物料抽过来 category_code 没法对照）
...
| 6 | 原系统是否区分 OrgCode（试点 3 单位过滤是否可行）| 原系统工程师 SQL 里体现 | 高 |
