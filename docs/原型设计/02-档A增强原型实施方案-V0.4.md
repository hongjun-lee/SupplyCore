...
const stateMachines = {
  'P-02': {
    ...
    states: {
      ...
      '待审': {
        on: {
          '审批通过': { target: '已审', actions: ['createTaskDrafts'] },
          '审批驳回': '已驳回'
        }
      },
      '已审': {
        // V0.4 修订（同事 P1.2）：'已审 → 已分解' 不再由人工触发，
        // 而是由 P-05 全部分解完毕后自动触发（见 markPlanDecomposedIfAllTasksReady）
    ...
    actions: {
      'createTaskDrafts': (plan) => {
        // V0.4 修订：自动创建 P-05 草稿（不是正式态），等计划员人工分解
        ...
        notify('PLANNER', '有新的采购任务草稿待分解', { plan_id: plan.id });
      }
    }
  },
  // ... 其余 7 个状态机
...
// V0.3 新增：计划员人工分解 / 合并后提交
linkage.on('P-05:草稿→已分解', async (tasks) => {
  // 计划员合并 / 拆分多个草稿为最终 P-05 任务
  // 触发采购方式分流：招采 / 直采 / 单一来源
...
| P-01 审批通过 | 审批实例完成 | 触发 P-02 计划聚合（D5 简化为 1:1）|
...
## 附录 A：方案页面名 ↔ 当前 prototype 实际页面名映射表（V0.3 新增）
...
完整版（含所有 17 页 / 50+ 个动作 / 详细 guards 校验函数 / 详细 actions 副作用代码）见 `04-档A动作联动矩阵-V0.1.md`（V0.4 计划写）。

---

## 附录 C：演示主线脚本骨架（V0.3 新增）
