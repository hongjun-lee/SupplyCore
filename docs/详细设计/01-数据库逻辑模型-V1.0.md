...
| `docs/详细设计/00-详细设计阶段规划-v0.2.md` | 详设拆分、深度边界、占位策略 |
| `docs/详细设计/开发实施引用索引-V1.0.md` | 开发参考优先级、开工条件、冲突处理 |
...
> **命名约定（2026-05-11 拍板 / 2026-05-12 增补）**：字段名 snake_case + 与 ABP `FullAuditedAggregateRoot<TKey>` 概念对齐。所有业务实体继承自定义抽象基类 `SupplyCoresFullAuditedAggregateRoot<TKey>`（在 ABP 基类上扩展 `created_org_id` + `sub_group_id` + `delete_reason` 三个详设独有字段；`sub_group_id` 于 2026-05-12 V1.1 增补，详见 `评审留痕/数据隔离边界sub_group_id修订建议清单-V0.1.md` 修订 #2）。完整映射表见 [`审计字段映射表-V0.1.md`](./审计字段映射表-V0.1.md)（文件名固定 V0.1，内容版本 V0.2）。
>
> **旧口径替换（同次评审决议）**：`created_at` → `creation_time` / `created_by` → `creator_id` / `updated_at` → `last_modification_time` / `updated_by` → `last_modifier_id` / `deleted_at` → `deletion_time` / `deleted_by` → `deleter_id` / `version_no` (integer) → `concurrency_stamp` (varchar(40) GUID 字符串)；详设 01 / 02 / 08-11 / 10A 共 7 个文档同步替换；F-14 接口定义表 `version_no` 字段（API 版本号）不属于审计字段，保留不动。

| 字段（DB 列名）| 类型 | 对应 ABP 实体属性 | 说明 |
| --- | --- | --- | --- |
| `creator_id` | uuid (FK→A-01 user_copy) | `CreatorId` | 创建人 |
| `creation_time` | timestamptz | `CreationTime` | 创建时间 |
| `last_modifier_id` | uuid? (FK→A-01 user_copy) | `LastModifierId` | 最后修改人 |
| `last_modification_time` | timestamptz? | `LastModificationTime` | 最后修改时间 |
| `created_org_id` | bigint? (FK→M-01 organization) | — *(详设独有)* | 创建人所属组织（细粒度过滤辅助）|
| `sub_group_id` | bigint? (FK→M-01 organization) | — *(详设独有，V1.1 新增)* | 数据所属二级集团（A-06 数据范围"以二级集团为隔离边界"一刀切主用字段）；写入时由 `created_org_id` 回溯 `M-01.sub_group_id` 确定；集团级共享主数据可为 NULL；详见 `评审留痕/数据隔离边界sub_group_id修订建议清单-V0.1.md` 修订 #2 + 原则 1-4 |
| `concurrency_stamp` | varchar(40) | `ConcurrencyStamp` | 乐观锁标识（GUID 字符串）；旧口径 `version_no` integer 已废弃 |

### 4.3 软删除（所有表必须）

| 字段（DB 列名）| 类型 | 对应 ABP 实体属性 | 说明 |
| --- | --- | --- | --- |
| `is_deleted` | boolean | `IsDeleted` | 软删除标志（默认 false）|
| `deleter_id` | uuid? (FK→A-01) | `DeleterId` | 删除人 |
| `deletion_time` | timestamptz? | `DeletionTime` | 删除时间 |
| `delete_reason` | varchar(500)? | — *(详设独有)* | 软删除原因（高敏感操作必填）|
...
```
business_date          业务发生日期（跨期处理用，与 creation_time 区分）
...
物料编码遵循 `docs/详细规则/物资编码规范文档.md` V1.8 的多级编码结构：大类 + 中类 + 小类 + 流水号 + 校验位。本文档不重复展开。
...
| C-09 | 月度预支付汇总 monthly_prepayment_summary | summary_id (PK), org_id (FK→M-01), summary_month, supplier_id (FK→M-09), summary_amount | summary_state: 待汇总/已汇总/已推付款/已回写实付/已作废 | 月末批处理生成；接口推送状态另用 interface_push_state |
...
| 编号 | 实体（中/英） | 关键字段 | 状态字段 | 备注 |
| --- | --- | --- | --- | --- |
| S-01 | 采购申请单 purchase_request | request_id (PK), request_no, org_id (FK→M-01), supplier_id (FK→M-09), contract_id (FK→C-02), fulfillment_type, direct_exception_workflow_id (FK→A-20), total_quantity, total_amount | request_state: 草稿/待审/已审/已驳回/部分下达/已下达/已关闭/已作废 | `fulfillment_type` 区分外购入库 / 直达 / 代储 / 外委检修 / 委托加工 |
...
| 编号 | 实体（中/英） | 关键字段 | 状态字段 | 备注 |
| --- | --- | --- | --- | --- |
| E-01 | 设备档案 equipment | equipment_id (PK), equipment_code, equipment_name, equipment_type, equipment_original_value, source_type, org_id (FK→M-01), warehouse_id (FK→M-02) | equipment_status: 可用/在用/租赁在用/维修中/停用/待报废/已报废；source_type: 采购入库/直达验收/外部购置/历史补录/租赁引入 | 跨域：可关联 S-05/S-03 来源；原值用于外委检修 40% 校验 |
...
| E-05 | 设备检修单 equipment_inspection | inspection_id (PK), equipment_id (FK→E-01), inspection_date, inspection_result, next_inspection_date, repair_mode, repair_amount, original_value, repair_contract_id (FK→C-02), repair_workflow_instance_id (FK→A-20) | — | 支持检修预警；外委检修启用 40% 原值上限与 WF-RPR-001 |
...
| E-07 | 租赁申请单 rental_request | request_id (PK), request_no, org_id (FK→M-01), usage_unit_id (FK→M-01), supplier_id (FK→M-09), equipment_type, expected_start_date, expected_end_date | request_state: 草稿/待审/已审/已驳回/已签合同/已作废 | — |
...
| E-13 | 租赁费用汇总表 rental_fee_summary | summary_id (PK), registration_id (FK→E-08), summary_month, monthly_fee, accumulative_fee | summary_state: 待汇总/已汇总/已推付款/已回写实付/已作废 | 月末批处理；推送付款/NC 后回写 |
...
| F-13 | 接口开关 interface_switch | switch_id (PK), switch_level, switch_target, switch_status, last_modifier_id, last_modification_time | switch_level: 模块级/接口级/组织级；switch_status: 开/关 | 支撑 14 模板的三级启停控制 |
...
| SY-02 | 系统配置项 system_config | config_id (PK), config_key, config_value, config_scope, config_description, last_last_modifier_id, last_last_modification_time | config_scope: 全局/组织/模块；config_status: 启用/停用 | 各类开关、默认值、阈值的统一存储；与 F-13 互补（F-13 专责接口开关） |
...
| `equipment_status` | 可用 / 在用 / 租赁在用 / 维修中 / 停用 / 待报废 / 已报废 | E-01 equipment | 设备生命周期 |
...
- 创建时间 `creation_time` 与业务日期 `business_date` 建立**升序索引**支撑报表范围查询
...
| V1.0a | 2026-05-10 | 同步 04 / 07 分卷政策回填：S-01 关键字段补 `fulfillment_type` 与 `direct_exception_workflow_id`，E-01 补 `equipment_original_value`，E-05 关键字段补外委检修金额、原值、合同与审批实例引用；实体总数不变。 |
| V1.1 | 2026-05-12 | 落地 `评审留痕/数据隔离边界sub_group_id修订建议清单-V0.1` 修订 #2：§4.2 审计字段表在 `created_org_id` 之后新增 `sub_group_id bigint? FK→M-01.organization` 详设独有字段（数据所属二级集团；A-06 数据范围"以二级集团为隔离边界"一刀切主用字段；写入由 created_org_id 回溯 M-01.sub_group_id 确定）；命名约定块同步登记基类扩展三字段（原两字段 + sub_group_id）。**联动：** 详设 02 V1.1 M-01 加同名字段、审计字段映射表 V0.2、Sprint 0 D1-5 抽象基类 SubGroupId 属性同期生效；实体总数不变。 |
