...
> **命名约定（2026-05-11 拍板）**：字段名 snake_case + 与 ABP `FullAuditedAggregateRoot<TKey>` 概念对齐。所有业务实体继承自定义抽象基类 `SupplyCoresFullAuditedAggregateRoot<TKey>`（在 ABP 基类上扩展 `created_org_id` + `delete_reason` 两个详设独有字段）。完整映射表见 [`审计字段映射表-V0.1.md`](./审计字段映射表-V0.1.md)。
>
...
| `created_org_id` | bigint? (FK→M-01 organization) | — *(详设独有)* | 创建人所属组织（A-06 数据范围过滤用）|
