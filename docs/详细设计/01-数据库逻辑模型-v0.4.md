...
| S-22 | 采购申请明细 purchase_request_line | line_id (PK), request_id (FK→S-01), material_id (FK→M-05), quantity, unit_id (FK→M-07), unit_price, line_amount | — | S-01 明细 |
| S-23 | 采购订单明细 purchase_order_line | line_id (PK), order_id (FK→S-02), request_line_id (FK→S-22), material_id (FK→M-05), quantity, unit_price, line_amount, delivery_date | line_state: 待到货/部分到货/全部到货/已关闭 | S-02 明细 |
...
| `request_state` | 草稿 / 待审 / 已审 / 已驳回 / 待执行 / 已执行 / 已作废 | 申请类单据（P-01/S-01/S-08/S-11/E-07） | — |
| `task_state` | 待执行 / 执行中 / 已完成 / 已驳回 | 任务类实体（P-05/S-15） | — |
...
| `return_state` | 待审 / 已审 / 已驳回 | 退货 / 退料 / 退租类单据（S-06/S-10/E-12） | — |
...
| `qualification_status` | 有效 / 已过期 / 待更新 | M-10 supplier_qualification | 资质有效期管理 |
...
| `batch_status` | 有效 / 过期 / 冻结 | M-15 material_batch | 批次生命周期 |
...
| `receipt_state` | 待验收 / 已验收 / 已拒收 | S-03 goods_receipt | 到货验收 |
| `inspection_state` | 待检 / 检验中 / 已检验 | S-04 quality_inspection | 质检 |
| `inspection_result` | 合格 / 不合格 / 让步接收 | S-04 quality_inspection | — |
| `estimate_state` | 暂估中 / 已冲销 / 已正式入账 | S-07 purchase_estimate | 暂估 |
| `transfer_order_state` | 待调拨 / 调出已发 / 待签收 / 已签收 / 已作废 / 已冲销 | S-12 transfer_order | 调拨 |
| `count_state` | 待盘 / 已盘 / 已审 | S-16 inventory_count | 盘点 |
| `declaration_state` | 待申请 / 待审 / 已审 / 已驳回 | S-19 scrap_declaration | 废旧认定 |
| `disposal_state` | 待处置 / 处置中 / 已完成 | S-20 scrap_disposal | 处置 |
| `disposal_type` | 报废 / 回收 / 变卖 / 销毁 | S-20 scrap_disposal | 04 决策 22 项确认 |
| `line_state` | 待执行 / 部分执行 / 已完成 / 已关闭 | 各类明细行实体（S-22~S-31 等） | 明细行级进度 |
