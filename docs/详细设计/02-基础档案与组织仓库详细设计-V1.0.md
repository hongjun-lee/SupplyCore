...
- 4.2 审计字段（creator_id / creation_time / last_modifier_id / last_modification_time / created_org_id / concurrency_stamp）
- 4.3 软删除（is_deleted / deleter_id / deletion_time / delete_reason）
...
| `sub_group_id` | bigint | — | NULL | NULL | — | FK→M-01.org_id | idx | 所属二级集团 org_id；自身即二级集团时 = org_id；集团根节点（org_level=1）为 NULL；与 Catio.platform.organizations.sub_group_id 对齐；数据范围"以二级集团为隔离边界"的一刀切主用字段 |
...
- `sub_group_id` 由 Nova 同步时**直接吃 Catio.platform.organizations.sub_group_id**（修订 #4 决议）；若 Catio 端为空（极少数老数据），物资侧按 `org_type='二级集团'` 沿 parent_id 回溯一次后回写，并触发 mapping_missing_alert 提示 Catio 端补数据。`parent_id` 变更触发的 `org_path` 重算同时重算 `sub_group_id`；物资侧不允许人工维护
- 集团根节点（`org_level=1`，辽宁能源）`sub_group_id IS NULL`；二级集团自身 `sub_group_id = org_id`；厂矿级（`org_level=3`）`sub_group_id` 必须指向其所在二级集团（`org_level=2`）的 `org_id`
...
2. **Nova 同步字段映射**：Catio.platform.organizations.sub_group_id → M-01.sub_group_id 直接同步，不做物资侧推导；Catio 端为空时按 `org_type='二级集团'` 回溯并触发告警（修订 #4 决议；详见 `评审留痕/数据隔离边界sub_group_id修订建议清单-V0.1.md`）
3. **数据权限根节点**：每个厂矿级组织（org_level=3）默认 `is_data_scope_root=true`，作为数据权限过滤的最小完整业务单元；`sub_group_id` 必须指向其所在二级集团
4. **跨组织业务**：跨组织调拨、共享仓引用、跨组织成本归集需在 M-16 / M-13 / S-12 中显式声明，不允许靠组织树推导
5. **历史数据保护**：组织 `org_id` 一经使用不允许重用；如 Nova 端 ID 重建，物资侧分配新 `org_id` 并标记前 ID `is_deleted=1`
...
**特别说明**：本表保留 `01-V1.0` 节 4.3 的软删除字段，但业务规则禁止删除历史日期的序列记录；历史号段仅作为台账长期保留。审计字段中 `concurrency_stamp` 用于乐观锁。

#### 4.9.2 取号并发控制

- **首选方案：行级锁（悲观锁）**——`SELECT ... FOR UPDATE` 锁定 `(prefix, date_key)` 行，UPDATE 后释放锁；保证并发下严格连续递增
- **备选方案：乐观锁（concurrency_stamp）**——读 current_value 和 concurrency_stamp，CAS 更新；冲突重试 3 次仍失败则降级到行级锁
...
erDiagram
    ...
    M-01 organization ||--o{ M-01 organization : "sub_group_id 自指（二级集团）"
...
| V1.1 | 2026-05-12 | 落地 `数据隔离边界sub_group_id修订建议清单-V0.1` 修订 #1 + #4：M-01 organization §4.1.1 全字段表 `org_path` 之后增加 `sub_group_id bigint NULL FK→M-01.org_id idx`（与 Catio.platform.organizations.sub_group_id 对齐，数据范围"以二级集团为隔离边界"的一刀切主用字段）；§4.1.1 特别说明补 3 条（同步规则 / 根节点取值 / 厂矿级取值约束）；§4.1.3 业务规则插入"Nova 同步字段映射"为 #2，原 #2-#4 顺移；§5.1 ERD 加 `sub_group_id 自指（二级集团）` 关系。**关键节奏：** 与 Sprint 0 D1-5 抽象基类 SubGroupId 属性 + 详设 01 V1.1 审计字段表 + 审计字段映射表 V0.2 同期生效。 |
