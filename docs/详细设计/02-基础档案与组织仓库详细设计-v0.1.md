...
本文档承接 `01-数据库逻辑模型-v0.4.md` 的跨模块骨架，把基础档案模块涉及的 9 个实体（组织、仓库、库区、货位、计量单位、单位换算、成本中心、成本中心映射、组织仓库关系、序列号生成器）的全字段、状态机、业务规则、接口规范、配置项和占位项固化下来。
...
| 输入文档 | 在本文档中的作用 |
| --- | --- |
| `docs/详细设计/01-数据库逻辑模型-v0.4.md` | 实体编号、共用约定（4.1-4.10）、状态值域、索引原则 |
...
下文字段表中**不重复列出**上述共用字段，只列实体专有字段；如某实体对共用约定有偏差（例如 SY-01 不需要软删除），在该实体的"特别说明"中显式注明。
...
| `status` | varchar | 16 | NOT NULL | `启用` | — | — | idx | 启用 / 停用 / 冻结 |
...
- **冻结状态**：货位特有 `冻结` 状态用于盘点期间局部冻结、危险物料临时管控等场景，停用与冻结独立处理
...
| `mapping_state` | varchar | 16 | NOT NULL | `未映射` | — | — | idx | 取值：未映射 / 待确认 / 已映射 |
...
| unit_code | unit_name | unit_type | precision | mapping_state | 备注 |
| --- | --- | --- | --- | --- | --- |
...
3. **NC 映射回填**：`mapping_state=已映射` 时 nc_unit_code 必须非空；NC 落地后由财务部门统一回填
...
**特别说明**：本表**不**遵守 4.3 软删除规则——历史日期的序列记录不删除，仅作为历史台账保留。审计字段中 `version_no` 用于乐观锁。
...
| TK | F-01 interface_task 接口任务 | 08 详设 |
...
| M-03B storage_location | status | 启用 / 停用 / 冻结 | 库区停用 → 自动停用；冻结独立处理（盘点/管控触发） |
