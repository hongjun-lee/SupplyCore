...
### 8.3.1 询价 / 竞争性谈判 — 供应商最低数量规则

> **业务依据：** 流程调研业务方答复 Q-02-6（2026-05-09）— 软提示，不阻断提交。

- 适用范围：`tender_type` ∈ {`询价`, `竞争性谈判`}
- 最低供应商数推荐：**3 家**（行业惯例）
- 校验级别：**软提示**（warning，**不阻断提交**）
  - 当 `invited_supplier_ids` / 实际响应供应商 < 3 家时，UI 显示警告："建议补充供应商至 3 家以上"
  - 业务员可在备注栏说明原因（如紧急采购 / 独家供应 / 集团目录内有限供应商）后继续提交
- 监控：详设 09 §六预警规则 ALR-PUR-001 在响应供应商 < 3 家时触发"重要"级提醒；月度报表 RPT-PUR-INSUFFICIENT 暴露不足比例（合规审计依据）。
- 直接采购（`tender_type=单一来源` / `直接采购`）本身**不要求 3 家**，不在本规则范围。
