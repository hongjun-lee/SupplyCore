...
本文档承接 `01-数据库逻辑模型-V1.0.md` 跨模块骨架与 `02-基础档案与组织仓库详细设计-V1.0.md`、`03-物料主数据与编码详细设计-V1.2.md` 的底座，把需求计划与采购协同模块涉及的 20 个实体的全字段、状态机、业务规则、接口规范、配置项和占位项固化下来。
...
| `docs/详细设计/03-物料主数据与编码详细设计-V1.2.md` | M-05 material_state 约束（不允许待申请/待审核状态物料进入采购流程） |
...
| T-08 | bid_response | 投标响应 | 全字段、4 状态机（提交→撤回/评标中→评定）、供应商投标主表（V1.2 新增）|
| T-09 | evaluation_result | 评标结果明细 | 全字段、关联 T-08 + T-06 评委，评分明细 + 综合得分（V1.2 新增）|
...
| `role_tags` | jsonb | — | NULL | `[]` | — | — | gin | 供应商业务角色标签数组（业务方调研 B2）：值域 `潜在 / 投标 / 中标 / 成交 / 历史合作`，与 `supplier_state` 生命周期正交，由系统在招采 / 中标 / 合同执行各环节自动累加；不影响准入约束，仅作为后评价输入维度 |
...
7. **业务角色标签自动累加**（业务方调研 B2）：M-09 `role_tags` 由系统在以下事件自动追加（去重 jsonb 数组追加）：
   - **潜在**：M-09 创建（`supplier_state=潜在`）时初始化；
   - **投标**：T-03 标包发标后供应商进入 T-05 中标候选时追加 `投标`；
   - **中标**：T-05 `verification_state=已验证` 且 `winning_quantity > 0` 时追加 `中标`；
   - **成交**：C-02 `contract_state=已签` 时追加 `成交`；
   - **历史合作**：合同 C-02 进入 `已完成` 状态后追加 `历史合作`，作为下次招标的优先邀请池。
   - 角色标签**只增不删**（除人工 A-11 高敏感清理）；同一标签不重复追加。报表输入：详设 09 RPT-SUP-002 履约率统计可按 `role_tags` 维度切片。
...
### 4.13a T-08 bid_response 投标响应（V1.2 新增）

> **V1.2 升版背景：** V1.1 §4.10.3 T-03 状态机引用了"投标响应"作为流标判定动作，但未单独定义 T-08 实体。Sprint 5α 评审时发现任务卡 D1-1 引用 T-08 BidResponse 而详设无定义，V1.2 补齐 T-08 全字段表与状态机；保留原 §4.14 起 S 系列编号不变，本节用 §4.13a 字母后缀编号插入。

#### 4.13a.1 全字段表

| 字段名 | 类型 | 长度/精度 | 空值 | 默认值 | 唯一 | 外键 | 索引建议 | 注释 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `bid_id` | bigint | — | NOT NULL | auto | PK | — | PK | 技术主键 |
| `bid_no` | varchar | 32 | NOT NULL | — | UQ | — | UQ | 投标响应号；前缀 `BR`（SY-01 取号） |
| `package_id` | bigint | — | NOT NULL | — | — | FK→T-03 | idx | 所属标包 |
| `supplier_id` | bigint | — | NOT NULL | — | — | FK→M-09 | idx | 投标供应商（须 `supplier_state=合格` 且在 T-03.qualified_supplier_ids 内）|
| `bid_date` | date | — | NOT NULL | — | — | — | idx | 投标日期 |
| `bid_amount` | decimal | (18,2) | NULL | — | — | — | — | 投标报价（不含税或含税口径见 unit_tax_rule_id） |
| `bid_amount_incl_tax` | decimal | (18,2) | NULL | — | — | — | — | 含税投标报价（如分别录入） |
| `delivery_promise_date` | date | — | NULL | — | — | — | — | 承诺交货日期 |
| `bid_document_file_id` | bigint | — | NULL | — | — | FK→SY-04 | — | 投标文件附件 |
| `bid_state` | varchar | 16 | NOT NULL | `已提交` | — | — | idx | 见 4.13a.2 |
| `withdraw_reason` | varchar | 512 | NULL | — | — | — | — | 撤回原因（仅 已提交→已撤回 时填写） |
| `evaluation_score` | decimal | (8,4) | NULL | — | — | — | — | 综合得分（评委评分 T-09 全部录入后回写） |
| `evaluation_rank` | smallint | — | NULL | — | — | — | — | 综合排名（评标完成后回写） |
| `evaluation_conclusion` | varchar | 16 | NULL | — | — | — | idx | 取值：中标 / 候选 / 落标 / 废标 |
| `import_source` | varchar | 16 | NOT NULL | `手工录入` | — | — | — | 取值：手工录入 / 平台导入（关联 T-06）|
| `platform_bid_code` | varchar | 64 | NULL | — | — | — | — | 招采平台原始投标编号（平台导入时回填）|

**唯一约束**：`(package_id, supplier_id)` 复合唯一 — 同一标包同一供应商只能有一条投标响应（如需重新投标，应先 已撤回 当前条目）。

#### 4.13a.2 状态机（4 状态）

```mermaid
stateDiagram-v2
    [*] --> 已提交 : 供应商提交投标响应（手工录入 / T-06 平台导入）
    已提交 --> 已撤回 : 供应商撤回（投标截止前；填 withdraw_reason）
    已提交 --> 评标中 : 投标截止后启动评标
    评标中 --> 已评定 : T-09 评分全部录入 + 综合得分/排名回写
    已评定 --> [*] : 综合得分驱动 T-05 中标结果生成
    已撤回 --> [*]
```

**状态约束**：
- `已提交 → 已撤回` 必须在 T-03.`bidding_deadline` 之前；过期后只能在 评标中 状态被判定为 废标
- `已提交 → 评标中` 需 T-03.`package_state=已发标` 且当前时间 ≥ `bidding_deadline`
- `评标中 → 已评定` 要求 T-09 评分记录数 ≥ T-06 评委组数量（业务方拍板：评委组人数由 T-09.evaluator_id 集合推断；本期不单独建评委表）

#### 4.13a.3 业务规则

1. **供应商资格校验**：`已提交` 时强校验 supplier_id 在 T-03.`qualified_supplier_ids` 列表内，且 `M-09.supplier_state=合格`（Active 强约束，同 Sprint 5 D1-D2 Material 加固风格）
2. **投标报价范围**：`bid_amount > 0`；如同时录入含税与不含税报价，二者差额应符合 `unit_tax_rule_id` 配置；本期一期口径允许仅录一种
3. **撤回时点**：`已提交 → 已撤回` 须在 T-03.`bidding_deadline` 之前；超期撤回拒绝
4. **平台导入幂等**：`import_source=平台导入` + `platform_bid_code` 形成业务幂等键；重复导入自动跳过（T-06 LogAsync 记录 fail_count）
5. **评分回写**：T-09 评分记录全部录入后，系统调用聚合函数计算 evaluation_score（加权平均）+ evaluation_rank（同标包内排序），回写本表；evaluation_conclusion 由业务员根据排名 + 评委结论手工录入（不自动判定 中标 vs 候选）
6. **流标判定关联**：T-03.状态机 `已发标 → 流标` 的"响应不足"判定基于本表 `bid_state ∈ (已提交, 已评定)` + `evaluation_conclusion ≠ 废标` 的供应商数量 ≥ T-03.`min_supplier_count`（详设 §4.10.5 V1.2 联动澄清）

---

### 4.13b T-09 evaluation_result 评标结果明细（V1.2 新增）

> **V1.2 升版背景：** 与 T-08 同期补齐。T-09 是评委针对每一份投标响应的评分明细，关联 T-08 投标响应 + T-06 招采平台对接日志（如评委通过平台录入则记入 T-06 LogAsync）。

#### 4.13b.1 全字段表

| 字段名 | 类型 | 长度/精度 | 空值 | 默认值 | 唯一 | 外键 | 索引建议 | 注释 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `eval_id` | bigint | — | NOT NULL | auto | PK | — | PK | 技术主键 |
| `bid_id` | bigint | — | NOT NULL | — | — | FK→T-08 | idx | 所属投标响应 |
| `evaluator_id` | bigint | — | NOT NULL | — | — | FK→A-01 | idx | 评委（业务方拍板：评委组人数由本字段去重集合推断，本期不单独建 T-X 评委表） |
| `score_dimension` | varchar | 32 | NOT NULL | — | — | — | idx | 评分维度（价格分 / 技术分 / 商务分 / 服务分 / 综合分） |
| `score` | decimal | (8,4) | NOT NULL | — | — | — | — | 该维度得分 |
| `weight` | decimal | (5,4) | NOT NULL | — | — | — | — | 权重（同维度同评委下，T-09 各 score 之和 × weight = T-08 evaluation_score 单评委贡献） |
| `evaluation_date` | date | — | NOT NULL | — | — | — | idx | 评分日期 |
| `comment` | varchar | 512 | NULL | — | — | — | — | 评委备注 |
| `import_source` | varchar | 16 | NOT NULL | `手工录入` | — | — | — | 手工录入 / 平台导入（T-06 LogAsync 关联） |

**唯一约束**：`(bid_id, evaluator_id, score_dimension)` 复合唯一 — 同一投标响应同一评委同一维度只能评一次

#### 4.13b.2 业务规则

1. **录入时点**：仅当所属 T-08.`bid_state=评标中` 时允许录入新评分；其他状态拒绝（前置校验，同 T-07 Material Active 风格）
2. **删除约束**：T-08.`bid_state=评标中` 时允许删除评分行（评委更正）；T-08 进入 `已评定` 后所有 T-09 记录冻结
3. **权重之和**：同评委对同一投标响应的各维度 weight 之和应 = 1.0（应用层 Submit 时校验）
4. **综合得分计算**：T-08 进入 `已评定` 前，系统按下式汇总：
   - 单评委综合分 = Σ (score × weight) over score_dimension
   - T-08.evaluation_score = avg(单评委综合分) over evaluator_id
   - T-08.evaluation_rank = 同 T-03 标包内按 evaluation_score 倒排
5. **评委去重**：同一 evaluator_id 对同一 bid_id 录入 T-09 时，系统校验同 (bid_id, evaluator_id) 下至少有 1 条评分（不强制全维度，weight 之和 = 1 即可）

#### 4.13b.3 与 T-06 联动

- 平台导入路径：T-06.LogAsync(`sync_direction=导入`, `sync_type=评标结果`) 写日志后批量 Insert T-09；T-06.success_count = 成功 Insert 数，fail_count = 业务规则拒绝数（含权重不和 / 评委超出 T-03 配置评委组等）
...
erDiagram
    ...
    T-03 tender_package ||--o{ T-08 bid_response : "投标响应"
    T-08 bid_response ||--o{ T-09 evaluation_result : "评分明细"
    T-08 bid_response }o--o| M-09 supplier : "投标供应商"
    T-09 evaluation_result }o--o| A-01 user : "评委"
...
| V1.1c | 2026-05-10 | 第四批补强 — 调研差异清单 E 类 (B2)：M-09 加 `role_tags jsonb` 供应商业务角色标签（潜在/投标/中标/成交/历史合作），与 supplier_state 生命周期正交，由系统在招采各环节自动追加；§4.1.3 业务规则补第 7 条业务角色标签自动累加触发事件。 |
| V1.2 | 2026-05-13 | Sprint 5α 评审 follow-up：补齐 T-08 bid_response 投标响应 + T-09 evaluation_result 评标结果明细两实体定义。V1.1 §4.10.3 T-03 状态机已引用"投标响应"作为流标判定动作但未定义实体表；Sprint 4/5 任务卡草案错把 T-08/T-09 当成已有实体编入而详设里没有，V1.2 补齐。§三 实体清单加 T-08 / T-09 两行；§4.13a / §4.13b 新增两节（用字母后缀避免 §4.14 S-01 重编号——S-01 已被外部 2 处强引用）；§六 ERD 关系加 T-08/T-09 与 T-03/M-09/A-01 关系；§4.10.5 流标判定与 T-08.bid_state 联动澄清。配合 Sprint 5α D1-D4 落 T-08/T-09 实体 + AppService + Controller。|
