...
本文档承接 `01-数据库逻辑模型-v0.4.md` 跨模块骨架与 `02-基础档案与组织仓库详细设计-v0.1.md`、`03-物料主数据与编码详细设计-v0.1.md` 的底座，把需求计划与采购协同模块涉及的 20 个实体的全字段、状态机、业务规则、接口规范、配置项和占位项固化下来。
...
| 输入文档 | 在本文档中的作用 |
| --- | --- |
| `docs/详细设计/01-数据库逻辑模型-v0.4.md` | 实体编号 M-09/10/11、P-01/06、P-02/03/04/05、T-01/02/03/04/05/06/07、S-01/22、S-02/23；共用约定 4.1-4.10 |
| `docs/详细设计/02-基础档案与组织仓库详细设计-v0.1.md` | SY-01 前缀清单、M-01/M-02/M-07/M-12/M-13 引用约定 |
| `docs/详细设计/03-物料主数据与编码详细设计-v0.1.md` | M-05 material_state 约束（不允许待申请/待审核状态物料进入采购流程） |
...
本篇所有实体表默认遵守 `01-v0.4` 节四的共用约定（主键策略、审计字段、软删除、业务状态字段、NC 接口字段、工作流字段、时间戳字段、多租户字段、附件字段、主表/明细表原则）。下文字段表中**不重复列出**共用字段；如某实体对共用约定有偏差，在该实体的"特别说明"中显式注明。
...
本篇新增以下业务单号前缀，须回写 `02-基础档案与组织仓库详细设计-v0.1.md` 节 4.9.5：
