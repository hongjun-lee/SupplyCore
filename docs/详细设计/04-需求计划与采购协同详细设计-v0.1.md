...
本文档承接 `01-数据库逻辑模型-v0.5.md` 跨模块骨架与 `02-基础档案与组织仓库详细设计-v0.2.md`、`03-物料主数据与编码详细设计-v0.2.md` 的底座，把需求计划与采购协同模块涉及的 20 个实体的全字段、状态机、业务规则、接口规范、配置项和占位项固化下来。
...
| 输入文档 | 在本文档中的作用 |
| --- | --- |
| `docs/详细设计/01-数据库逻辑模型-v0.5.md` | 实体编号 M-09/10/11、P-01/06、P-02/03/04/05、T-01/02/03/04/05/06/07、S-01/22、S-02/23；共用约定 4.1-4.10 |
| `docs/详细设计/02-基础档案与组织仓库详细设计-v0.2.md` | SY-01 前缀清单、M-01/M-02/M-07/M-12/M-13 引用约定 |
...
本篇所有实体表默认遵守 `01-v0.5` 节四的共用约定（主键策略、审计字段、软删除、业务状态字段、NC 接口字段、工作流字段、时间戳字段、多租户字段、附件字段、主表/明细表原则）。下文字段表中**不重复列出**共用字段；如某实体对共用约定有偏差，在该实体的"特别说明"中显式注明。
...
| `status` | varchar | 16 | NOT NULL | `有效` | — | — | idx | 取值：有效 / 已过期 / 待更新 / 已作废 |
| `remarks` | varchar | 255 | NULL | — | — | — | — | 备注 |
...
- 到期前 **90 天**：`status` 自动置为 `待更新`，触发 R-04 `SUPPLIER_QUAL_EXPIRY_NEAR` 预警，通知供应商管理员
- 到期前 **30 天**：升级通知物资管理
- 到期当日：`status` 自动置为 `已过期`
...
**特别说明**：T-06 为纯日志型实体，**不**遵守软删除规则（日志只增不删不改）；无业务状态字段；写入后不允许修改。
...
| `line_state` | varchar | 16 | NOT NULL | `待下达` | — | — | idx | 取值：待下达 / 已下达 / 已关闭 |
...
| `nc_push_state` | varchar | 16 | NOT NULL | `待推送` | — | — | idx | NC 接口推送状态（取自 4.4 共用约定） |
...
| `line_state` | varchar | 16 | NOT NULL | `待到货` | — | — | idx | 取值：待到货 / 部分到货 / 全部到货 / 已关闭 |
...
- `已关闭`：所有 S-23 行的 `line_state=全部到货 OR 已关闭`
- `已冲销`：属高敏感操作（A-11），须走专项审批；冲销后在 S-21 inventory_transaction 写入冲销流水

#### 4.15.4 业务规则

1. **NC 接口触发**：`order_state=已下达` 后，`nc_push_state` 从 `待推送` → `推送中`，由 F-01 interface_task 驱动推送 NC 采购订单接口（F-14 接口定义；**NC 未落地阶段 `nc_push_state` 保持 `待推送` 且 `F-13 interface_switch` 为关闭**）
...
4. **到货跟踪**：S-03 到货验收创建时回写 `S-23.received_quantity`，驱动 `S-23.line_state` 迁移；`line_state` 变化自动汇总到 `S-02.order_state`
...
| M-10 supplier_qualification | status | 有效 / 已过期 / 待更新 / 已作废 | 每日凌晨任务驱动；90 天前→待更新，到期日→已过期 |
...
- NC 回执 F-03 回写 S-02 `nc_voucher_no / nc_push_state`
...
本篇新增以下业务单号前缀，须回写 `02-基础档案与组织仓库详细设计-v0.2.md` 节 4.9.5：
