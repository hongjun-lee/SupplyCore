...
本文档承接 `01-数据库逻辑模型-v0.6.md` 跨模块骨架与 `02-基础档案与组织仓库详细设计-v0.2.md`、`03-物料主数据与编码详细设计-v0.2.md` 的底座，把需求计划与采购协同模块涉及的 20 个实体的全字段、状态机、业务规则、接口规范、配置项和占位项固化下来。
...
| 输入文档 | 在本文档中的作用 |
| --- | --- |
| `docs/详细设计/01-数据库逻辑模型-v0.6.md` | 实体编号 M-09/10/11、P-01/06、P-02/03/04/05、T-01/02/03/04/05/06/07、S-01/22、S-02/23；共用约定 4.1-4.10 |
...
本篇所有实体表默认遵守 `01-v0.6` 节四的共用约定（主键策略、审计字段、软删除、业务状态字段、NC 接口字段、工作流字段、时间戳字段、多租户字段、附件字段、主表/明细表原则）。下文字段表中**不重复列出**共用字段；如某实体对共用约定有补充说明，在该实体的"特别说明"中显式注明。
...
**特别说明**：T-06 为纯日志型实体，保留 `01-v0.6` 节 4.3 的软删除字段，但业务规则禁止删除或修改日志记录；日志只增不删不改，无业务状态字段。
...
本篇新增以下业务单号前缀，已回写 `02-基础档案与组织仓库详细设计-v0.2.md` 节 4.9.5：
...
| V0.2 | 2026-05-02 | 按 01-v0.6 上位口径修正 5 项：T-06 保留软删除字段但业务禁止删除/修改日志；M-10 `status` 改为 `qualification_status`；S-22 `line_state` 改为 `request_line_state`；S-23 `line_state` 改为 `order_line_state`；S-02 `nc_push_state` 统一为 `interface_push_state`。同步将 AD/PT/TA 前缀说明改为已回写 02-v0.2。 |
