...
本文档承接 `01-数据库逻辑模型-V1.0.md` 跨模块骨架与 `04-需求计划与采购协同详细设计-V1.0.md` 的上游协同，把合同与资金模块涉及的 11 个实体的全字段、状态机、业务规则、接口规范、配置项和占位项固化下来。
...
| `docs/详细设计/04-需求计划与采购协同详细设计-V1.0.md` | T-05 中标结果驱动合同；S-01 合同执行来源；S-02 NC 接口触发点与合同约束；M-09 供应商状态约束 |
...
> **C-02 表格全字段升级在下次详设小版本（V1.1）时统一加入；本版仅在业务规则层面登记。**
...
- **详设 10 §7 高敏感清单**：履约保证金"已没收"动作 = SENS-CON-003（合同没收保证金，强制留痕 + 二次确认）—— 建议下版本加入
