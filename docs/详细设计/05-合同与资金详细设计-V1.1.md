...
- C-02 / C-11 字段补充建议（V1.x 升级时落表）：
  - C-02 `service_subtype varchar(32)` 服务合同子类
  - C-11 `equipment_id bigint NULL FK→E-01` 关联设备（仅外委检修类合同必填）
  - C-02 `overlimit_reason varchar(512) NULL` 超阈原因（仅超阈值时必填）
  - C-02 `overlimit_approval_id bigint NULL FK→A-20` 关联超阈审批实例
