...
| `equipment_id` | bigint | — | NULL | — | — | FK→E-01 | idx | 关联设备（外委检修类合同必填，V1.3 §8.7 价格上限规则联动） |
| `overlimit_reason` | varchar | 512 | NULL | — | — | — | — | 外委检修合同超 40% 原值阈值原因说明（超阈时必填） |
| `overlimit_approval_id` | bigint | — | NULL | — | — | FK→A-20 | idx | 外委检修合同超阈值审批实例（WF-CON-OVERLIMIT-001） |
...
- 价格基数：合同标的设备的**原值**（来自详设 07 设备档案 E-01 `equipment_original_value` 字段，并在 E-05 `original_value` 形成检修申请快照；外委检修合同必须在 C-02 contract 关联具体设备 `equipment_id (FK→E-01)`，合同明细 C-11 如需拆分多设备可在后续版本扩展行级设备字段）
...
- C-02 字段：
  ...
  - **C-02 `equipment_id bigint NULL FK→E-01`** ✅ V1.3 已加入 §4.2.1 正式字段表（仅外委检修类合同必填）
  - **C-02 `overlimit_reason varchar(512) NULL`** ✅ V1.3 已加入 §4.2.1 正式字段表；超阈时必填
  - **C-02 `overlimit_approval_id bigint NULL FK→A-20`** ✅ V1.3 已加入 §4.2.1 正式字段表；关联 WF-CON-OVERLIMIT-001 审批实例
...
| V1.3 | 2026-05-13 | Sprint 5β B1 小升版：§4.2.1 C-02 全字段表正式加入 `equipment_id`（FK→E-01，外委检修类合同必填）、`overlimit_reason`、`overlimit_approval_id` 三字段；§8.7.1 将外委检修 40% 原值上限的设备关联口径从 C-11 建议收口到 C-02 主表；§8.7.3 字段补充建议全部标记为已落 V1.3，支撑后续 B4 C-02 实体扩展与 SENS-CON-004 / WF-CON-OVERLIMIT-001。|
