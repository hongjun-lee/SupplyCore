...
本文档承接 `01-数据库逻辑模型-v0.6.md` 跨模块骨架与 `04-需求计划与采购协同详细设计-v0.2.md` 的上游协同，把合同与资金模块涉及的 11 个实体的全字段、状态机、业务规则、接口规范、配置项和占位项固化下来。
...
| 输入文档 | 在本文档中的作用 |
| --- | --- |
| `docs/详细设计/01-数据库逻辑模型-v0.6.md` | 实体编号 C-01~C-11；共用约定 4.1-4.10；合同域状态值域节八；口径冲突处理节十二 |
...
本篇所有实体表默认遵守 `01-v0.6` 节四的共用约定（主键策略、审计字段、软删除、业务状态字段、NC 接口字段、工作流字段、时间戳字段、多租户字段、附件字段、主表/明细表原则）。下文字段表中**不重复列出**共用字段；如某实体对共用约定有偏差，在该实体的"特别说明"中显式注明。
...
| `到货验收` | S-03 `receipt_state=已验收` | 自动（关联 S-03.receipt_id） |
| `入库完成` | S-05 `receipt_state=已审`（入库审核通过） | 自动（关联 S-05.receipt_id） |
...
| 入库匹配 | 非预付款类型须有对应的已审入库单 | `S-05.receipt_state=已审` 且 `S-05.contract_id=C-02.contract_id` |
...
- 三单匹配：S-05 `is_invoice_arrived / receipt_state` 作为 C-08 提交前提条件
