...
> **业务依据：** 流程调研业务方答复 Q-07-4（2026-05-09）。

- 委托加工材料发出后，在物资公司账面上**走独立"委托加工材料"科目**，区分于自有库存（1403 原材料）。
- 库存层落点：建议在 S-13 `material_stock` 增 `inventory_type` 字段（值域：`自有 / 委托加工 / 代储`），便于资产负债表呈现委托加工资产。
- 财务凭证：发出材料时借"委托加工物资"，贷"原材料"（详见详设 08 BIZ-019 委托加工财务触发）。
- 加工完毕入库：借"自有库存"，贷"委托加工物资"（含加工费 + 运杂费分摊，待业务方 Q-12-7 确认是否独立科目）。
