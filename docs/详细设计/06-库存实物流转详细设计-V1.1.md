...
本文档承接 `01-数据库逻辑模型-V1.0.md` 跨模块骨架，以及 `02-基础档案与组织仓库详细设计-V1.0.md`、`03-物料主数据与编码详细设计-V1.2.md`、`04-需求计划与采购协同详细设计-V1.1.md`、`05-合同与资金详细设计-V1.1.md` 的上游口径，把库存实物流转模块涉及的到货、验收、入库、出库、退货、退料、调拨、盘点、废旧处置、库存台账与库存事务流水固化到字段级、状态级和规则级。
...
| `docs/详细设计/03-物料主数据与编码详细设计-V1.2.md` | M-05 物料启停约束、M-15 批次状态、HG/HX 强制批次与 FIFO 口径 |
| `docs/详细设计/04-需求计划与采购协同详细设计-V1.1.md` | S-01/S-22/S-02/S-23 采购申请与采购订单口径；订单行到货状态回写 |
| `docs/详细设计/05-合同与资金详细设计-V1.1.md` | S-05/S-25 采购入库回写 C-11 执行数量与 C-04 付款节点条件；三单匹配前置 |
...
stateDiagram-v2
    ...
    待检 --> 品种检验中 : 检验员接单（先核品种）
    品种检验中 --> 数量检验中 : 品种合格
    品种检验中 --> 不合格转待验区 : 品种不合格 / 短路终止后续检验
    数量检验中 --> 质量检验中 : 数量合格
    数量检验中 --> 不合格转待验区 : 数量不合格 / 短路终止后续检验
    质量检验中 --> 已检验 : 质量合格 / 不合格 / 让步接收
    质量检验中 --> 不合格转待验区 : 质量严重不合格
    不合格转待验区 --> 已检验 : 填写不合格判定 + 处置建议
    待检 --> 已作废 : 来源到货作废
    已检验 --> [*] : 合格/让步数量生成 S-05；不合格数量生成 S-06 或拒收闭环
```

#### 4.3.1 三类验收串行短路（业务方 Q-05-1 答复 2026-05-09）

> **业务依据：** 业务方 Q-05-1 拍板"同意串行短路"。详设落地：质检环节按"品种 → 数量 → 质量"三类串行执行，任一类不合格即转"待验区"，不再继续执行后续类别检验，避免质检资源浪费。

- **串行顺序**：品种检验 → 数量检验 → 质量检验（不可并行、不可乱序）
  1. **品种检验**：核对到货物料编码 / 规格 / 型号是否与 S-03 / S-23 一致；不合格 → 短路终止 → `inspection_state=不合格转待验区`，`unqualified_reason=品种不符`
  2. **数量检验**：核对到货数量是否与 S-03 一致（含允许超收 / 短缺范围）；不合格 → 短路终止 → `inspection_state=不合格转待验区`，`unqualified_reason=数量不符`
  3. **质量检验**：抽检 / 全检 / 外检的质量判定；可能结果：合格 / 不合格 / 让步接收
- **状态机字段**：S-04 `inspection_phase varchar(16) NULL`（建议新增字段）值域：`品种检验中 / 数量检验中 / 质量检验中 / 已完成`，记录当前所处阶段
- **短路效率**：当前阶段不合格立即跳转"不合格转待验区"，不再触发下一阶段；节省质检员 / 检验设备资源
- **物理待验区**：不合格物资转入仓库的"待验区"（物理隔离区域），不进财务账（联动 §7.6.5 不合格物资不产生暂估），等待"退货 / 让步接收 / 返修 / 报废"决策
- **联动详设 09**：新增 RPT-QC-001 质检结果与待验区物资统计表，按月输出三类验收的不合格率分布
...
| 项 | 默认值 | 配置位置 | 调整责任方 |
|---|---|---|---|
| 暂估窗口（暂估入库 → 发票冲减最长间隔）| **6 个月** | SY-02 `INVENTORY_TENTATIVE_WINDOW_DAYS` = 180 | 财务（高敏感配置） |
| 首次超期预警提前天数（D-90）| 90 天 | SY-02 `INVENTORY_TENTATIVE_WARN_DAYS_FIRST` = 90 | 财务 + 业务部门 |
| 二次超期预警提前天数（D-30）| 30 天 | SY-02 `INVENTORY_TENTATIVE_WARN_DAYS_SECOND` = 30 | 财务 + 业务部门 |

> **与调研对照：** 流程 05/12 调研约定"最长不超 12 个月"。**详设按行业标杆缩为 6 个月**（业务方 Q-05-2 V1.1 答复 2026-05-09 已确认 6 个月 + D-90/D-30 二段预警）；超过 6 个月仍未冲减时，按 7.6.4 触发预警与强制对账。

#### 7.6.4 超期处理（D-90 / D-30 / D / D+30 四阶段）

- **D-90（暂估第 3 个月）**：详设 09 §六 ALR-INV-006 触发"暂估首次超期预警"（一般级），提醒业务员跟进发票到位
- **D-30（暂估第 5 个月）**：详设 09 §六 ALR-INV-006B 触发"暂估二次超期预警"（重要级），同时推送业务员 + 财务，强制业务员发起供应商对账
- **D-0（暂估第 6 个月，到达 `INVENTORY_TENTATIVE_WINDOW_DAYS`）**：升级为 ALR-INV-007"暂估超期阻断预警"（紧急级），对账完成前阻断该供应商的新采购订单审批
- **D+30（超期 30 天后仍未冲减）**：自动转入详设 09 §六"暂估超期物资"月度报表，提交财务月度核对，必要时启动专项对账
...
#### 7.7.1 直达物资验收角色与审批层级（业务方 Q-08-2 / Q-08-4 答复 2026-05-09）

> **业务依据：** Q-08-4 答"按厂矿流程验收" + Q-08-2 答"业务主管 / >100 万 总经理"。详设落地：直达验收角色由厂矿自主流程定义；非直达走直达通道时按金额阈值升级审批。

**直达验收角色**：

- 直达物资由厂矿（使用单位）按其内部验收流程自主组织，物资公司不强制三方联合验收
- S-03 / S-24 到货验收明细的 `inspector_id` 由厂矿指定（默认厂矿仓管 / 厂矿使用部门负责人）
- 物资公司业务员**仅负责接收厂矿验收结果**（S-03 `goods_receipt_state=已验收` 后回写到物资侧），不参与厂矿现场验收过程
- 验收角色字段建议（V1.x 升级落表）：S-03 `inspection_party varchar(32) NOT NULL DEFAULT '厂矿自主'`（验收主导方：厂矿自主 / 物资公司主导 / 三方联合 / 第三方独立）

**非直达走直达通道审批层级**：

- 触发条件：物料原本不在《可直达物资目录》（M-05 字段或物料分类层定义），但实际业务中需要走直达通道
- 审批分档（按 100 万阈值）：
  - **金额 < 100 万**：物资公司业务主管审批（A-08 模板：WF-DIRECT-EXC-001 业务主管 1 节点）
  - **金额 ≥ 100 万**：升级总经理审批（A-08 模板：WF-DIRECT-EXC-002 业务主管 → 总经理 2 节点）
- 配置项：SY-02 `DIRECT_EXC_GENERAL_MANAGER_THRESHOLD=1000000`（100 万阈值，可由业务部门 + 财务调整）
- 联动详设 10：建议新增 WF-DIRECT-EXC-001 / WF-DIRECT-EXC-002 模板
...
| 06-09 | **报废核销审批层级（业务方 Q-10-5 未拍板）** | S-19/S-20、详设 10 §6.x | 业务方答"完全方案待会议讨论"。详设暂按推断口径占位：金额分档 < 5 万业务主管 / 5-50 万分管领导 / > 50 万企业负责人 / > 500 万集团；阈值落 SY-02 配置项 `SCRAP_LEVEL_DEPT=50000` / `SCRAP_LEVEL_VICE_GM=500000` / `SCRAP_LEVEL_GROUP=5000000`；具体方案待会议讨论后回填 | 业务方专项会议拍板 |
...
| V1.1a | 2026-05-10 | 调研政策业务确认核查 — 第一/三批 4 项内容补强：(B1) §7.6.3 暂估时限规约改为 D-90 / D-30 / D / D+30 四阶段（业务方 Q-05-2 V1.1 答复）；(B4) §4.3 状态机改三类验收串行短路 + §4.3.1 全新一节（业务方 Q-05-1）；(B5+C2) §7.7.1 直达物资验收角色 `inspection_party` + 非直达走直达通道审批分档 100 万阈值（业务方 Q-08-2/Q-08-4）；(C6) §十"业务确认占位项"加 06-09 报废核销审批层级占位（业务方 Q-10-5 未拍板，按推断金额分档 < 5 万 / 5-50 万 / >50 万 / >500 万）。 |
