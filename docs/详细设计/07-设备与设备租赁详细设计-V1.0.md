...
本文档承接 `01-数据库逻辑模型-V1.0.md` 的设备租赁域骨架，以及 `06-库存实物流转详细设计-V1.1.md` 中采购入库、直达验收、库存受限和废旧处置口径，把设备管理与设备租赁管理涉及的 14 个实体的字段、状态机、业务规则、接口触发点、配置项和占位项固化下来。
...
**外委检修补充字段（当 `inspection_type=外委检修` 时启用）：**

| 字段名 | 类型 | 长度/精度 | 空值 | 默认值 | 外键 | 说明 |
| --- | --- | --- | --- | --- | --- | --- |
| `repair_mode` | varchar | 16 | NULL | — | — | 自修 / 集团内部 / 外委 |
| `repair_amount` | decimal | (18,2) | NULL | — | — | 检修预算或合同金额 |
| `original_value` | decimal | (18,2) | NULL | — | — | 设备原值，用于 40% 上限校验 |
| `repair_contract_id` | bigint | — | NULL | — | FK→C-02 | 外委检修合同 |
| `repair_supplier_id` | bigint | — | NULL | — | FK→M-09 | 外委检修单位 |
| `repair_workflow_instance_id` | bigint | — | NULL | — | FK→A-20 | WF-RPR-001 审批实例 |
...
4. 外委检修必须先判断自修 / 集团内部单位能力；内部不能满足时须上传回函并走 WF-RPR-001，审批通过后方可签订外委检修合同。
5. 外委检修价格原则上不得超过设备原值 × `OUTSOURCED_REPAIR_PRICE_CAP_RATIO`（默认 40%）；超过时须在 WF-RPR-001 中走总经理节点并留专项说明。
6. 更换下来的配件由使用单位统一回收，回收单作为 E-05 必备附件；竣工验收合格后才能进入检修结算。
