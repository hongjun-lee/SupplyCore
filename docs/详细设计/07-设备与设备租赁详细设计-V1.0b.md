...
本文档承接 `01-数据库逻辑模型-V1.0.md` 的设备租赁域骨架，以及 `06-库存实物流转详细设计-V1.1.md` 中采购入库、直达验收、库存受限和废旧处置口径，把设备管理与设备租赁管理涉及的 15 个实体（V1.0b 在 V1.0a 14 个实体基础上拆出 E-05a）的字段、状态机、业务规则、接口触发点、配置项和占位项固化下来。
...
| 输入文档 | 在本文档中的作用 |
| --- | --- |
| `docs/详细设计/01-数据库逻辑模型-V1.0.md` | 实体编号 E-01~E-14、设备租赁域 ERD、设备与库存边界、共用字段约定 |
| `docs/详细设计/02-基础档案与组织仓库详细设计-V1.0.md` | M-01 组织、M-02 仓库、M-09 供应商、M-16 组织仓库关系、SY-01 前缀清单、SY-04 通用附件 |
| `docs/详细设计/03-物料主数据与编码详细设计-V1.1.md` | M-05 物料档案（设备类物料属性标识）、M-07 计量单位 |
| `docs/详细设计/05-合同与资金详细设计-V1.1.md` | 租赁合同 C-02、付款节点 C-04、付款计划 C-07、付款申请 C-08 的引用边界 |
| `docs/详细设计/06-库存实物流转详细设计-V1.1.md` | 设备来源入库、库存受限/已租出、废旧处置与 S-21 库存事务口径 |
...
| E-05 | repair_application | 外委检修申请单（V1.0b 语义升级） | 全字段、6 状态机、WF-RPR-001 审批前置、外委检修闭环 |
| E-05a | equipment_inspection_record | 设备检修执行记录（V1.0b 新增） | 全字段、4 状态机、由 E-05 进入执行阶段自动生成 |
...
本篇所有实体默认遵守 `01-V1.0` 节四共用约定：主键策略、审计字段、软删除、审批字段、时间戳字段、附件字段、数据权限字段和多租户预留字段。下文字段表不重复列出共用字段；如某实体有特殊要求，在“特别说明”中单独说明。
...
```mermaid
flowchart LR
    A[采购入库/直达验收/外部购置] --> B[E-01 设备建档 Draft]
    B --> C[Active 已启用]
    C --> D[InUse 在用]
    D --> C
    C --> E[Leased 租赁在用]
...
1. 设备建档可以来自 S-05 采购入库、S-03 大型设备直达验收或外部已有设备补录；建档完成后处于 `Draft` 状态，启用后方可进入业务流转。
2. E-01 是设备生命周期台账，不作为库存数量事实源；库存数量事实仍以 S-13/S-14/S-21 为准。
3. 设备状态变更必须留痕，跨组织权属变更必须审批；V1.0b 起 Equipment 状态机扩为 7 状态，所有迁移须由业务单据驱动（详 §5.1.1）。
...
| `equipment_status` | varchar | 16 | NOT NULL | `Draft` | — | — | idx | V1.0b 7 状态枚举：Draft（草稿）/ Active（已启用）/ InUse（在用）/ Leased（租赁在用）/ UnderRepair（维修中）/ PendingScrap（待报废）/ Scrapped（已报废） |
...
#### 5.1.1 状态机（V1.0b 7 状态）

```mermaid
stateDiagram-v2
    [*] --> Draft : 建档创建
    Draft --> Active : 启用确认（E-02 状态变更）
    Active --> InUse : 投入使用（E-02 领用/投用）
    InUse --> Active : 收回闲置（E-02 状态变更）
    Active --> Leased : 起租确认（E-09 rental_start）
    InUse --> Leased : 起租确认（E-09 rental_start）
    Leased --> Active : 退租交接完成（E-12 + E-14）
    Active --> UnderRepair : 送修（E-05 RepairApplication.Start）
    InUse --> UnderRepair : 送修（E-05 RepairApplication.Start）
    UnderRepair --> Active : 修复完成（E-05a EquipmentInspectionRecord.Complete）
    Draft --> PendingScrap : 报废申请审批通过（E-06）
    Active --> PendingScrap : 报废申请审批通过（E-06）
    InUse --> PendingScrap : 报废申请审批通过（E-06）
    Leased --> PendingScrap : 报废申请审批通过（E-06，需先退租）
    UnderRepair --> PendingScrap : 报废申请审批通过（E-06）
    PendingScrap --> Scrapped : 处置完成（E-06.Dispose；终态）
```

**转换矩阵（V1.0b 7 状态全量）：**

| # | From | To | 触发动作 | 触发实体 / 联动钩子 |
| --- | --- | --- | --- | --- |
| 1 | Draft | Active | 启用确认 | E-02 status_change（启用） |
| 2 | Active | InUse | 领用 / 投用 | E-02 status_change（投用） |
| 3 | InUse | Active | 收回闲置 | E-02 status_change（收回） |
| 4 | Active | Leased | 起租确认 | E-09 rental_start.Confirm |
| 5 | InUse | Leased | 起租确认 | E-09 rental_start.Confirm |
| 6 | Leased | Active | 退租交接完成 | E-12 rental_return.Approve + E-14 handover.Confirm |
| 7 | Active | UnderRepair | 送修 | E-05 RepairApplication.Start（已审 → 执行中） |
| 8 | InUse | UnderRepair | 送修 | E-05 RepairApplication.Start |
| 9 | UnderRepair | Active | 修复完成 | E-05a EquipmentInspectionRecord.Complete |
| 10 | Draft | PendingScrap | 报废审批通过 | E-06 scrapping.Approve |
| 11 | Active | PendingScrap | 报废审批通过 | E-06 scrapping.Approve |
| 12 | InUse | PendingScrap | 报废审批通过 | E-06 scrapping.Approve |
| 13 | Leased | PendingScrap | 报废审批通过（需先退租） | E-06 scrapping.Approve（前置 E-12+E-14） |
| 14 | UnderRepair | PendingScrap | 报废审批通过 | E-06 scrapping.Approve |
| 15 | PendingScrap | Scrapped | 处置完成（终态） | E-06 scrapping.Dispose |
...
4. **状态受控**：`equipment_status` 不允许直接手工改值，必须由 E-02/E-05/E-05a/E-06/E-09/E-12/E-14 等业务单据驱动；V1.0b 新增 InUse/UnderRepair 两个中间状态后，所有非法迁移须由 Application 服务层抛 `EquipmentStateTransitionDenied` 阻断。
5. **租赁占用**：`equipment_status=Leased` 时，不允许再次起租；如需报废须先完成 E-12 退租 + E-14 交接。
6. **维修中受限**：`equipment_status=UnderRepair` 时，不允许起租（E-09）、状态变更为 InUse、权属变更（E-03）；仅允许由 E-05a 完成事件驱动回到 Active。
7. **Draft → 业务流转前置**：`equipment_status=Draft` 时不允许 E-04 维修保养、E-05 RepairApplication、E-07~E-14 租赁链路；必须先经 E-02 启用至 Active。
...
### 5.5 E-05 repair_application 外委检修申请单（V1.0b 语义升级）

**V1.0b 升版说明：** 原 V1.0a 单实体 `equipment_inspection`（4 状态）已沿 Sprint 5b B3 落地为审批型 `RepairApplication`（6 状态），承担"申请—审批—合同—执行入口"职责；执行阶段的工时/验收/签字/结算等过程留痕拆出至 §5.5a `EquipmentInspectionRecord`（V1.0b 新增 4 状态执行实体）。代码层命名空间 `Nova.SupplyCores.Equipment.RepairApplications`（Sprint 5b 已落 Wave 39）。

| 字段名 | 类型 | 长度/精度 | 空值 | 默认值 | 唯一 | 外键 | 索引建议 | 注释 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `repair_application_id` | bigint | — | NOT NULL | auto | PK | — | PK | 技术主键 |
| `repair_application_no` | varchar | 32 | NOT NULL | — | UQ | — | UQ | 前缀 `EIN`（沿用原检修单前缀，V1.0b 不重发号段） |
| `equipment_id` | bigint | — | NOT NULL | — | — | FK→E-01 | idx | 设备 |
| `inspection_type` | varchar | 32 | NOT NULL | — | — | — | idx | 定期检修 / 临时检修 / 安全检查 / 专项检查 / 外委检修 |
| `planned_inspection_date` | date | — | NOT NULL | — | — | — | idx | 计划检修日期 |
| `repair_mode` | varchar | 16 | NOT NULL | — | — | — | — | 自修 / 集团内部 / 外委 |
| `repair_amount` | decimal | (18,2) | NULL | — | — | — | — | 检修预算或合同金额 |
| `original_value` | decimal | (18,2) | NULL | — | — | — | — | 设备原值快照，用于 40% 上限校验 |
| `repair_contract_id` | bigint | — | NULL | — | — | FK→C-02 | idx | 外委检修合同 |
| `repair_supplier_id` | bigint | — | NULL | — | — | FK→M-09 | idx | 外委检修单位 |
| `repair_workflow_instance_id` | bigint | — | NULL | — | — | FK→A-20 | idx | WF-RPR-001 审批实例 |
| `internal_unable_reply_file_id` | bigint | — | NULL | — | — | FK→SY-04 | — | 内部单位不能检修回函 |
| `disassembly_image_file_ids` | jsonb | — | NULL | — | — | SY-04 ID 列表 | — | 拆解后影像附件 ID 列表 |
| `completion_acceptance_file_id` | bigint | — | NULL | — | — | FK→SY-04 | — | 设备检修竣工验收单 |
| `replaced_parts_recycle_file_id` | bigint | — | NULL | — | — | FK→SY-04 | — | 更换配件回收单 |
| `repair_settlement_file_id` | bigint | — | NULL | — | — | FK→SY-04 | — | 设备检修结算单 |
| `application_state` | varchar | 16 | NOT NULL | `草稿` | — | — | idx | 草稿 / 待审 / 已审 / 已驳回 / 执行中 / 已完成 |

#### 5.5.1 状态机（V1.0b 审批型 6 状态）
...
stateDiagram-v2
    ...
    待审 --> 已驳回 : 审批驳回
    已驳回 --> 草稿 : 修改后重提
    已审 --> 执行中 : Start（生成 E-05a 执行记录草稿，设备 → UnderRepair）
    执行中 --> 已完成 : E-05a 全部完成回写
```
...
| From | To | 触发动作 | 联动钩子 |
| --- | --- | --- | --- |
| 草稿 | 待审 | Submit | 校验 40% 原值上限 + WF-RPR-001 启动 |
| 待审 | 已审 | Approve（终审节点签批） | A-20 工作流实例 Approve |
| 待审 | 已驳回 | Reject | 工作流回退 + 通知申请人 |
| 已驳回 | 草稿 | Reopen | 申请人修改 |
| 已审 | 执行中 | Start | **自动生成 E-05a EquipmentInspectionRecord 草稿** + E-01 状态 → UnderRepair |
| 执行中 | 已完成 | Complete | E-05a 所有执行记录 Complete 后回写；E-01 经 E-05a.Complete 钩子回 Active |

#### 5.5.2 业务规则

1. 外委检修必须先判断自修 / 集团内部单位能力；内部不能满足时须上传回函并走 WF-RPR-001，审批通过后方可签订外委检修合同。
2. 外委检修价格原则上不得超过设备原值 × `OUTSOURCED_REPAIR_PRICE_CAP_RATIO`（默认 40%）；超过时须在 WF-RPR-001 中走总经理节点并留专项说明。
3. 更换下来的配件由使用单位统一回收，回收单作为 E-05 必备附件；竣工验收合格后才能进入检修结算（结算单字段填齐后方可 `Complete`）。
4. `application_state=已审 → 执行中` 时**自动创建 E-05a EquipmentInspectionRecord 草稿**（状态 `待执行`），并将 E-01 状态由 Active/InUse 切换至 UnderRepair；联动钩子由 RepairApplication.Start Domain Event 触发。
5. 申请受理设备必须处于 Active 或 InUse；Draft / Leased / UnderRepair / PendingScrap / Scrapped 状态下不允许新建 RepairApplication（`EquipmentNotActive` 守护规则）。
6. 检修超期预警由 `09-报表预警与AI能力详细设计` 承接，本文仅固化字段来源。

---

### 5.5a E-05a equipment_inspection_record 设备检修执行记录（V1.0b 新增）

**V1.0b 拆分动机：** 将原 E-05 内置的"执行结果 / 实际工时 / 验收 / 签字"等过程数据独立为执行型实体，与审批型 RepairApplication 解耦；一条 RepairApplication 进入"已审 → 执行中"时自动派生一条 EquipmentInspectionRecord 草稿，由检修执行人接续填写。

| 字段名 | 类型 | 长度/精度 | 空值 | 默认值 | 唯一 | 外键 | 索引建议 | 注释 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `inspection_record_id` | bigint | — | NOT NULL | auto | PK | — | PK | 技术主键 |
| `inspection_record_no` | varchar | 32 | NOT NULL | — | UQ | — | UQ | 前缀 `EIR` |
| `repair_application_id` | bigint | — | NOT NULL | — | — | FK→E-05 | idx | 来源外委检修申请单 |
| `equipment_id` | bigint | — | NOT NULL | — | — | FK→E-01 | idx | 设备（冗余字段，便于按设备聚合查询） |
| `actual_start_time` | datetime | — | NULL | — | — | — | idx | 实际开始时间（Start 动作回写） |
| `actual_complete_time` | datetime | — | NULL | — | — | — | idx | 实际完成时间（Complete 动作回写） |
| `labor_hours` | decimal | (10,2) | NULL | — | — | — | — | 工时记录（小时） |
| `actual_labor_cost` | decimal | (18,2) | NULL | — | — | — | — | 实际工费（外委结算 / 内部人工成本核算） |
| `engineer_signature_file_id` | bigint | — | NULL | — | — | FK→SY-04 | — | 工程师签字附件 |
| `inspection_result` | varchar | 32 | NULL | — | — | — | idx | 合格 / 需复修 / 建议停用 / 建议报废 |
| `acceptance_person_id` | bigint | — | NULL | — | — | FK→A-01 | idx | 验收人 |
| `acceptance_opinion` | varchar | 512 | NULL | — | — | — | — | 验收意见 |
| `record_attachment_file_ids` | jsonb | — | NULL | — | — | SY-04 ID 列表 | — | 现场照片 / 检测报告等附件 ID 列表 |
| `next_inspection_date` | date | — | NULL | — | — | — | idx | 下次检修日期（用于回写 E-01） |
| `record_remarks` | varchar | 1024 | NULL | — | — | — | — | 备注 |
| `record_state` | varchar | 16 | NOT NULL | `待执行` | — | — | idx | 待执行 / 执行中 / 已完成 / 已取消 |

#### 5.5a.1 状态机（V1.0b 执行型 4 状态）

```mermaid
stateDiagram-v2
    [*] --> 待执行 : RepairApplication.Start 自动生成
    待执行 --> 执行中 : Start（实际开始）
    执行中 --> 已完成 : Complete（验收签字后）
    待执行 --> 已取消 : Cancel（申请被撤回或终止）
    执行中 --> 已取消 : Cancel（异常终止，需审批留痕）
```
...
| From | To | 触发动作 | 联动钩子 |
| --- | --- | --- | --- |
| (initial) | 待执行 | （由 RepairApplication.Start 自动派生） | E-05 RepairApplication.Start Domain Event |
| 待执行 | 执行中 | Start（执行人开工） | 回写 `actual_start_time` |
| 执行中 | 已完成 | Complete | 必填 `inspection_result/acceptance_person_id/acceptance_opinion`；**回写 E-01 状态：UnderRepair → Active + `next_inspection_date`**；触发 E-05 RepairApplication.Complete |
| 待执行 / 执行中 | 已取消 | Cancel | 异常终止；E-01 状态由 UnderRepair 回退至原状态（业务侧补 E-02 留痕） |

#### 5.5a.2 业务规则

1. **来源约束**：E-05a 不允许独立创建，必须由 E-05 RepairApplication 进入"已审 → 执行中"时自动派生；同一 `repair_application_id` 在未取消的前提下仅允许一条有效 E-05a。
2. **完成必填**：进入 `已完成` 必须同时具备 `actual_start_time / actual_complete_time / inspection_result / acceptance_person_id / acceptance_opinion`；缺一阻断 Complete。
3. **设备状态回写**：E-05a `Complete` 钩子负责将 E-01 状态由 `UnderRepair` 切回 `Active`，并写 `next_inspection_date`；`Cancel` 钩子由业务侧补 E-02 手工恢复状态。
4. **结果联动**：`inspection_result=建议停用 / 建议报废` 时自动生成待办，提示发起 E-02 状态变更或 E-06 报废处置。
5. **审计留痕**：所有状态迁移写 A-11/A-13 审计链路；验收签字附件作为必备审计要件。
...
| 触发事件 | 联动结果 |
| --- | --- |
| E-01 建档完成 | 生成设备台账（`Draft`），经 E-02 启用至 `Active` 后可进入状态变更、权属变更、维修检修、报废或租赁流程 |
| E-02 状态变更确认 | 回写 E-01 `equipment_status`（Draft↔Active / Active↔InUse 等） |
| E-03 权属变更审批通过 | 回写 E-01 `org_id/usage_unit_id/responsible_person_id` |
| E-05 RepairApplication.Start（已审 → 执行中） | **自动派生 E-05a EquipmentInspectionRecord 草稿**（`待执行`）；回写 E-01 `equipment_status=UnderRepair` |
| E-05a EquipmentInspectionRecord.Complete | 回写 E-01 `equipment_status: UnderRepair → Active` + `next_inspection_date`；触发 E-05 RepairApplication.Complete；必要时提示 E-02/E-06 |
| E-06 报废处置审批通过 | 回写 E-01 `equipment_status=PendingScrap`；处置完成置 `Scrapped` |
| E-09 起租确认 | 回写 E-08 `registration_state=在租`，E-01 `equipment_status=Leased` |
| E-10 续租审批通过 | 回写 E-08 `planned_end_date` |
| E-11 停租执行 | 回写 E-08 `registration_state=已停租`，影响 E-13 计费天数 |
| E-12 + E-14 退租交接完成 | 回写 E-12/E-08/E-01（`equipment_status: Leased → Active`），完成租赁闭环 |
...
| EIN | E-05 repair_application | 外委检修申请单（V1.0b 语义升级，前缀沿用） |
| EIR | E-05a equipment_inspection_record | 设备检修执行记录（V1.0b 新增） |
...
| `OUTSOURCED_REPAIR_APPROVAL_THRESHOLD_LOW` | 10000 | 固定/政策修订 | 设备管理部门 | 外委检修 1 万元审批分档 |
| `OUTSOURCED_REPAIR_APPROVAL_THRESHOLD_HIGH` | 100000 | 固定/政策修订 | 设备管理部门 | 外委检修 10 万元审批分档 |
| `OUTSOURCED_REPAIR_PRICE_CAP_RATIO` | 40% | 30%-50% | 设备管理 + 物资部门 | 外委检修价格原则上不超设备原值比例 |
| `OUTSOURCED_REPAIR_MONTHLY_REPORT_DAY` | 20 | 1-28 | 设备管理部门 | 外委检修完成情况月度报送截止日 |
...
| V1.0a | 2026-05-10 | 同步政策 01 / 02 与流程 14：E-05 补外委检修字段、审批前置、40% 原值上限、配件回收 / 竣工验收 / 结算附件，以及外委检修 1 万 / 10 万审批分档配置。 |
| V1.0b | 2026-05-13 | **Sprint 6b Day 1 升版**：① E-05 拆为 §5.5 `repair_application`（审批型 6 状态，承接 Sprint 5b B3 已落代码）+ 新增 §5.5a `equipment_inspection_record`（执行型 4 状态，10+ 字段，由 E-05.Start 自动派生）；② Equipment 状态机由 V1.0a 4 状态（可用/在用/维修中/已报废 等中文枚举）扩展为 V1.0b 7 状态英文枚举（Draft/Active/InUse/Leased/UnderRepair/PendingScrap/Scrapped），§5.1.1 补 15 边转换矩阵 + mermaid 状态图 + 每条边的业务单据触发钩子；③ §4.1 设备生命周期流程图同步 7 状态；④ §6.1 联动表新增 E-05/E-05a/E-12+E-14 钩子；⑤ §6.5 SY-01 前缀新增 `EIR`（E-05a）。E-05 单号前缀 `EIN` 沿用不重发号。本版仅升版详设 07；下游 01/02 ERD 节点同步及代码层 §5.5a 实体落地为 Sprint 6b Day 2-3 任务。 |
