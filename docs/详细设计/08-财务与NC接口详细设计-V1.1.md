...
| `docs/详细设计/03-物料主数据与编码详细设计-V1.2.md` | M-14 物料 NC 映射、M-15 批次、物料状态与映射拦截 |
| `docs/详细设计/05-合同与资金详细设计-V1.1.md` | C-07/C-08/C-10 付款计划、付款申请、支付回写与预付款接口触发 |
| `docs/详细设计/06-库存实物流转详细设计-V1.1.md` | S 域入库、出库、调拨、盘点、废旧、库存事务触发点 |
...
| BIZ-005 | 领料出库（物资公司自用） | S-09 | 出库单审核通过、`issuance_type` ∈ 自用类 | 准实时 | 借 6602，贷 1403 |
| BIZ-005A | **对厂矿销售出库** | S-09 | 出库单审核通过、收货方为厂矿（集团内部用户单位） | 准实时 | **借 1122 应收账款，贷 6001 主营业务收入 + 2221 销项税** |
...
| BIZ-019 | 委托加工财务触发 | 委托加工单/扩展单据 | 发出或加工费确认 | 准实时 | 借"委托加工物资"独立科目，贷"原材料"（业务方 Q-07-4 确认 2026-05-09） |
| BIZ-020 | 付款执行（应付消减） | C-08 + C-10 | 付款申请审批通过 + 实付完成 | 准实时 | 借 2202 应付账款，贷 1002 银行存款；预付款场景由 BIZ-014/015 处理，不走 BIZ-020（业务方 Q-13-4 确认 2026-05-09：付款流程仅消减应付，应付挂账时点已在 BIZ-001/002 完成） |

说明：BIZ-019 一期若尚未形成独立委托加工业务单据，可先在 F-14/F-13 中保留接口定义和开关，业务落地时由模块详设补齐来源单据。BIZ-020 由 C-10 付款执行台账触发；C-10 应回写 NC 凭证号 + 实付时点 + 银行流水号，详设 05 §C-10 字段已含 `payment_voucher_no`。

> **BIZ-005 / BIZ-005A 拆分说明（Q-MERGE-1 联动）：** 流程调研业务方答复 Q-MERGE-1（2026-05-09）确认：物资公司对厂矿（集团内部用户单位）出库**按外部销售实质开票**（税法视同销售）。因此 S-09 出库单需根据收货方分流到不同凭证模板：
> - 物资公司自用 / 内部消耗 → BIZ-005（借 6602 销售费用，贷 1403 原材料）
> - 对厂矿销售 → BIZ-005A（借 1122 应收账款，贷 6001 主营业务收入 + 2221 销项税）
>
> S-09 出库单分流字段：`usage_unit_id`（使用单位）— 当 `usage_unit_id` 是厂矿（集团内部用户单位）时走 BIZ-005A；当是物资公司自身组织时走 BIZ-005。详细分流规则见详设 05 §8.4 / 详设 06 §7.7。
...
| `last_modifier_id` | bigint | — | NOT NULL | — | — | FK→A-01 | — | 更新人 |
| `last_modification_time` | timestamp | — | NOT NULL | 当前时间 | — | — | — | 更新时间 |
...
| V1.1 | 2026-05-09 | 同步 P0 业务答复：新增 / 明确 BIZ-005A 对厂矿销售出库凭证模板（借应收账款，贷主营业务收入 + 销项税），并预置 BIZ-019 委托加工财务触发口径；S-09 出库按使用单位分流到自用出库或对厂矿销售出库。 |
| V1.1a | 2026-05-10 | 调研政策业务确认核查 — 第一批 1 项：(C5) BIZ 表新增 BIZ-020 付款执行（应付消减）凭证模板（借 2202 应付账款，贷 1002 银行存款），与详设 05 §8.2.1 应付挂账时点联动（业务方 Q-13-4）；BIZ-001/BIZ-002 应付挂账时点 + BIZ-014/015 预付款转应付路径已通过详设 05 §8.2.1 显式说明，本表保持原条目不变。 |

---

## 十五、一句话结论

08 详设的核心是把“业务事实如何安全进入 NC、NC 结果如何可信回到物资系统”做成一套可控链路：接口定义可配置、任务可追踪、报文可审计、回执可回写、失败可重推、差异可对账、封账可控制、反结有审批；对厂矿销售出库与委托加工财务触发已预置口径，在 NC 尚未落地时先固化模型和开关，正式启用时再补齐 endpoint、科目和联调细节。
