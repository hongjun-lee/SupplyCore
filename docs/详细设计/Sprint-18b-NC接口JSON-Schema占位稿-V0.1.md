...
### 1.5 NCC OpenAPI 公开资料参考（V0.1.1 新增）

> **来源**：2026-05-14 网上检索（CSDN / 博客园用友 NCC OpenAPI 文档）— **以下信息为 SupplyCores 单边采集，待 NC 端正式确认**。

#### 1.5.1 NCC OpenAPI 请求路径模式

NCC OpenAPI 路径约定：`http://NCC_IP:port/nccloud/api/{模块}/{业务组件}/{操作}`
- 凭证 GL 模块：`/nccloud/api/gl/voucher/{add|delete|query|update}`
- 销售订单：`/nccloud/api/so/saleorder/save`
- 客户档案：`/nccloud/api/uapbd/customermanage/customer/add`

> [⚠️ NC 端待确认 NC-1-1]：NC 端是否走 NCC OpenAPI Gateway？还是直连 NC65 业务接口？两种路径鉴权 / 包装差异较大

#### 1.5.2 NCC OpenAPI 三层包装请求（与 §1.1 直接业务 endpoint 假设不同）

```json
{
  "openapi_ncurl": "http://ncc-host:port",
  "openapi_uri": "/nccloud/api/gl/voucher/add",
  "openapi_data": { /* 真实凭证 payload，见 §1.5.4 */ }
}
```

> [⚠️ NC 端待确认 NC-1-2]：是否需要 SupplyCores 在客户端做三层包装？还是 NC 端 OpenAPI Gateway 自动转换？如果 SupplyCores 端要包装，需加 NccOpenApiAdapter 层（~1 PD）

#### 1.5.3 NCC OpenAPI 响应结构（与 §1.2 `{code, message, voucherNo}` 假设不同）

```json
{
  "data": { /* NCC 业务返回（含 pk_voucher / num 凭证号等）*/ },
  "state": 1,      // 1=成功 / 2=失败（数字，非字符串 "0000"）
  "message": "..."
}
```

> [⚠️ NC 端待确认 NC-1-3]：state=1/2 是 NCC OpenAPI Gateway 层结构。如果直连 NC65 业务接口，响应可能是另一种结构（业务码可能是 NC 端自定义字段如 success/errorCode）。需要 NC 端明确响应 schema

#### 1.5.4 NCC gl_voucher 凭证表头字段（NC 数据库表）

| NC 字段 | 中文含义 | 类型 | SupplyCores 占位稿对照 |
|---|---|---|---|
| `pk_voucher` | 凭证主键 UUID | string(20) | 我们设计的 `voucherNo` ≠ 此字段（pk_voucher 是 NC 内部主键，非业务凭证号）|
| `pk_org` | 财务组织 | string(20) | 对应 SupplyCores `org_id` 但**类型不同**（NC 用 UUID） |
| `pk_accountingbook` | 核算账簿 | string(20) | 占位稿未覆盖 — **新增字段** |
| `pk_vouchertype` | 凭证类型 | string(20) | 占位稿未覆盖 — **新增字段**（如"记账凭证"/"收款凭证"）|
| `year` | 会计年度 | int | 对应 SupplyCores `periodCode` 解析的年 |
| `period` | 会计期间（月）| int | 对应 SupplyCores `periodCode` 解析的月 |
| `pk_prepared` | 制单人 UUID | string(20) | SupplyCores 用 `Guid userId`，需做 UUID 映射 |
| `prepareddate` | 制单日期 | date | 对应 `transactionDate` 业务日期 |
| `billdate` | 单据日期 | date | 同上 |
| `explanation` | 摘要 | string(200) | 对应 SupplyCores `summary` |
| `num` | 业务凭证号 | string(20) | **这才对应我们的 `voucherNo`**（NC 端生成的业务凭证编号）|

#### 1.5.5 NCC gl_detail 凭证分录字段（NC 数据库表）

| NC 字段 | 中文含义 | 类型 | SupplyCores F-12 NcAccountRule 对照 |
|---|---|---|---|
| `pk_detail` | 分录主键 | string(20) | 占位稿未覆盖 |
| `pk_voucher` | 关联凭证主键 | string(20) | 占位稿未覆盖 |
| `pk_accasoa` | **科目 UUID** | string(20) | **核心差异** — F-12 现存 `DebitAccount="1403"` / `CreditAccount="2202"` 编码，**实际 NC 用 pk_accasoa UUID**（每个 NC 实例不同）|
| `explanation` | 摘要 | string(200) | 同 voucher 表头 |
| `debitamount` | 借方原币金额 | decimal(28,8) | 对应 `debitAmount` |
| `creditamount` | 贷方原币金额 | decimal(28,8) | 对应 `creditAmount` |
| `debitquantity` | 借方数量 | decimal(28,8) | 占位稿未覆盖 — **新增字段**（数量类凭证用）|
| `creditquantity` | 贷方数量 | decimal(28,8) | 占位稿未覆盖 |
| `pk_currtype` | 币种 UUID | string(20) | 一期人民币默认，需要 NC 端 CNY 对应的 UUID |
| `assid` | 辅助核算关联 | string(20) | 占位稿未覆盖 — **新增字段**（多维辅助核算）|

> [⚠️ NC 端待确认 NC-1-4]：F-12 NcAccountRule 应该存 NC 科目编码（"1403"）还是 NC 内部 pk_accasoa UUID？建议：编码作为 cici/财务方可读字段；运行时通过 MD-001 物料映射类似机制做编码 → UUID 转换（SupplyCores 内部缓存 MD-003 科目对照字典）
>
> [⚠️ NC 端待确认 NC-1-5]：pk_currtype 一期硬编码 CNY UUID 还是 SY-02 字典化？
>
> [⚠️ NC 端待确认 NC-1-6]：F-03 InterfaceReceipt 现存 NcVoucherNo 单字段；NC 实际返 pk_voucher（内部 UUID）+ num（业务凭证号）双字段，需要拆 NcVoucherPk + NcVoucherNo 双字段（涉及 Wave 91 schema 升级）

#### 1.5.6 NCC OpenAPI 鉴权方式

公开资料未明确鉴权机制（**可能不是 OAuth2 client_credentials**）。NC 端常见模式：
- **Sign 签名**：`access_token` + `app_id` + `timestamp` + HMAC-SHA256 签名
- **AppKey + Secret**：自定义 token 透传

> [⚠️ NC 端待确认 NC-1-7]：NC 端鉴权方式 — 如果不是 OAuth2 client_credentials，Sprint 17a NcOAuth2TokenService 可能需要替换为 NcSignTokenService（~0.5 PD 实现）。**关键问题**：cici 与 NC 端确认时建议明确告知"SupplyCores 已实现 OAuth2 client_credentials 标准流，NC 端能否提供该方式？或者必须 Sign 签名？"
...
### 5.1.X NCC OpenAPI 公开资料对照（V0.1.1 新增，7 条）

> 来源：2026-05-14 网上检索 NCC OpenAPI 公开资料，SupplyCores 单边采集，待 NC 端正式确认。

| 编号 | 问题 |
|---|---|
| NC-1-1 | NC 端是否走 NCC OpenAPI Gateway / 还是直连 NC65 业务接口（两种路径鉴权/包装差异大）|
| NC-1-2 | SupplyCores 端是否需要做三层包装（openapi_ncurl/openapi_uri/openapi_data）？还是 Gateway 自动转换 |
| NC-1-3 | 响应结构是 `{code, message, voucherNo}`（SupplyCores 假设）还是 `{data, state, message}` 的 NCC 模式 |
| NC-1-4 | F-12 NcAccountRule 应该存 NC 科目编码（"1403"）还是 NC 内部 pk_accasoa UUID — 建议加 MD-003 科目对照字典做编码→UUID 转换 |
| NC-1-5 | pk_currtype 币种 UUID 一期硬编码 CNY UUID 还是 SY-02 字典化 |
| NC-1-6 | F-03 InterfaceReceipt 现 NcVoucherNo 单字段 vs NC 实际返 pk_voucher（UUID）+ num（业务凭证号）双字段 — 是否升级 Wave 91 schema |
| NC-1-7 | NC 端鉴权方式 — 是否支持 OAuth2 client_credentials（Sprint 17a 已实现）/ 还是必须 Sign 签名（HMAC+timestamp，需新建 NcSignTokenService ~0.5 PD） |
...
> **§5 总条数更新**：原 53 条（53 ⚠️ ID）+ NC-1-1~7 = **60 条**
...
| V0.1.1 | 2026-05-14 | 网上检索 NCC OpenAPI 公开资料嵌入 — §1.5 加 NCC 实际路径模式 / 三层包装 / state 响应 / gl_voucher 表头 / gl_detail 分录字段 + §5.1.X 加 NC-1-1~7 类条目（鉴权 / Gateway / pk_accasoa UUID / 凭证号双字段等关键差异）总条数 56→60 |
...
## 八、NCC OpenAPI 公开资料溯源（V0.1.1 新增）

§1.5 / §5.1.X 信息来源 — 2026-05-14 WebSearch：

- [用友NCC产品API使用指南 - 博客园](https://www.cnblogs.com/qliang-cloud/p/14968260.html)
- [NCC的openApi介绍、NCC二开 - CSDN](https://blog.csdn.net/chen892446383/article/details/123726588)
- [NC6X/NCC总账凭证与辅助的数据表关系 - 博客园](https://www.cnblogs.com/Pro-Cyon/p/15020313.html)
- [Open API开放平台 - 用友开发者中心](https://developer.yonyou.com/openAPI)

> **重要声明**：以上信息为 SupplyCores 单边采集，**不代表阜矿集团 NC 实例的实际接口形态**。可能存在以下偏差：
> - NCC 标准 OpenAPI 与 NC65 / NC63 直连业务接口差异
> - 阜矿 NC 实例的定制开发 / 二开适配
> - NC 端运维实际开放的鉴权方式
>
> 最终 schema 以 NC 端正式反馈为准；本占位稿 V0.1.1 §1.5 / §5.1.X 仅作为 cici 与 NC 端沟通时的"备选格式"参考。
