...
### v0.10 档 A 一期 · Week 1 框架层（按 02 V0.4 §八 路线图）

按 `docs/原型设计/02-档A增强原型实施方案-V0.4.md` 启动一期实施，第 1 周交付**引擎层 4 个新文件 + 1 个烟雾测试页**：
...
#### 5. `prototype/_engine-test.html` — 烟雾测试页
开发面向，验证引擎层端到端。6 个验证卡：
- ① Seed 主数据状态（实体 + 记录数）
- ② P-01 CRUD + 状态迁移（草稿 → 待审 → 已审）
- ③ P-02 审批 → linkage 自动生成 P-05 草稿 → 分解路径分流
- ④ 状态机引擎调试（已注册 SM + 当前 P-01 可执行 events）
- ⑤ 联动总线调试（已订阅事件 + 触发计数）
- ⑥ BroadcastChannel 跨页验证（在另一标签页操作 → 这里实时收到）

#### 路线图位置

| 周 | 内容 | 状态 |
|---|---|---|
| **Week 1** | 框架层：store / statemachine / linkage / seed | **进行中**（5 天中已交付 1-3 天产出，对应 02 V0.4 §八 第 1 周天 1-3）|
...
剩余 Week 1 待办：天 4 `mock-nc.js`（NC 推送 mock 1-2 秒延迟 + 5% 失败率 + 重推机制）+ 天 5 `chrome.js` 升级 + `roles.js` 6 角色 + `ui-helper.js`。
...
- **v0.10 (2026-05-10)** — 启动档 A 一期 Week 1 引擎层（按 02 V0.4 §八 第 1 周天 1-3）：新增 `assets/store.js`（LocalStorage CRUD + BroadcastChannel 跨页广播 + snapshot/rollback 事务）、`assets/statemachine.js`（9 状态机：P-01/P-02/P-05/T-01/T-03/C-02/S-02/S-03/S-04/S-05）、`assets/linkage.js`（V0.4a 收口：状态机不做副作用，linkage 唯一负责级联；预定义 P-02:已审/P-05:草稿→已分解/T-05:已验证/C-02:已签/S-05:已审/F-01:推送失败 6 个关键联动）、`assets/seed-data.js`（主数据 M-01/02/05/09/12 + F-13 NC 开关 + P-01/02/03 业务种子）、`prototype/_engine-test.html`（6 个验证卡的烟雾测试页）。共 40 页 HTML + 7 个 assets。剩余 Week 1 天 4-5（mock-nc.js + chrome.js 升级 + roles.js + ui-helper.js）pending。
